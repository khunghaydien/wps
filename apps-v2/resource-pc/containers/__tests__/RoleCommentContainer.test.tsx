...
describe('RoleCommentContainer Test', () => {
  ...
  it('dispatch event when hardbook', async () => {
    component = mount(
      <Provider store={store}>
        <RoleCommentContainer primaryAction={ROLE_ACTIONS.HARDBOOK} />
      </Provider>
    );
  ...
  it('dispatch event when softbook', async () => {
    component = mount(
      <Provider store={store}>
        <RoleCommentContainer primaryAction={ROLE_ACTIONS.SOFTBOOK} />
      </Provider>
    );
    const dialog = component.find(RoleCommentDialog).props();
    await act(async () => {
