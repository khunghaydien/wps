...
const useSumTimeOfOriginalTaskList = (): number => {
  const originalTaskList = useSelector(
    (state: State) => state.ui.dailySummary.originalTaskList
  );
  return React.useMemo(() => {
    return originalTaskList
      .filter((task) => task.isDirectInput)
      .reduce((prev, task) => {
        return prev + (task.taskTime || 0);
      }, 0);
  }, [originalTaskList]);
};

const mapStateToProps = (state: State) => ({
  empId: state.entities.user?.id || undefined,
  ...
  originalTasks: state.ui.dailySummary.originalTaskList,
  isTemporaryWorkTime: state.ui.dailySummary.isTemporaryWorkTime,
  useTimeAutoWorkingHourAllocation:
    state.ui.dailySummary.useTimeAutoWorkingHourAllocation,
...
const TaskCardContainer: React.FC = () => {
  ...
  const timeOfExternalTaskTime = useSumTimeOfOriginalTaskList();
  ...
  const checkBeforeOpenAutoHoursAllocationResultDialog =
    React.useCallback(() => {
      return dispatch(
        DailySummaryActions.checkBeforeOpenAutoHoursAllocationResultDialog(
          tasks,
          originalTasks
        )
      );
    }, [dispatch, tasks, originalTasks]);

  const onApplyAllocatedResult = React.useCallback(
    (
      result: AutoHoursAllocationResult[],
      surplusTimeRegistrationSetting: AutoHoursAllocationDictSurplusTime
    ) => {
      return dispatch(
        DailySummaryActions.applyAllocatedResult(
          originalTasks,
          result,
  ...
  return (
    <TaskCard
      ...
      useTimeAutoWorkingHourAllocation={useTimeAutoWorkingHourAllocation}
      ...
      onApplyAllocateResult={onApplyAllocatedResult}
      empId={empId}
    />
  );
};
