import EmployeeRepository, {
  PRODUCT_CATEGORY_OF_RECORD_ACCESS as PRODUCT,
  SearchQuery,
...
const searchByRepositoryRequest =
  (param: SearchQuery) => (dispatch: AppDispatch) => {
    return dispatch(
      withLoading(
        () =>
          EmployeeRepository.searchBySettingLimit(
            // NOTE: 勤怠機能以外での使途が生じた場合は、ダイアログのマウント側でproductを注入する拡張が必要になります
            { isHadRecordAccess: true, product: PRODUCT.ATTENDANCE, ...param },
            LIMIT_NUM
          ),
        (result) => {
          dispatch(
