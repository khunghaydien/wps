...
export default class Grid extends React.Component<Props, State> {
  ...
  isSelectedAllChecked() {
    const { data, selected, maxSelection, idKey } = this.props;
  ...
  renderRow(rowData: {
    ...
    const rowClass = rowData.rowClass;
    ...
    return (
      <BodyRow
        ...
        showCheckBox={this.props.showCheckBox || rowData.showCheckbox}
      ...
      <label className={inputClassName} onClick={this.onClickLabel}>
        <Tooltip
          ...
          content={TextUtil.template(
            msg().Appr_Msg_MaxSelected,
            this.props.maxSelection ?? 100
          )}
        >
          <input
            type="checkbox"
