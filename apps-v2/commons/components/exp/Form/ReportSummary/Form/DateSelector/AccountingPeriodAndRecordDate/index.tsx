...
export default class AccountingPeriodAndRecordDate extends React.Component<Props> {
  ...
  buildSelectedValue() {
    ...
    // When user has created a report while the AP was off, there is no existing selectedAccountingPeriod.
    // Now the admin turn on the AP, when it reaches this code, it does not find selectedAccountingPeriod,
    // so we need to display the Record Date from the report.
    if (isEmpty(recordDate)) recordDate = get(expReport, 'accountingDate');
