...
> {
  ...
  componentDidUpdate() {
    if (this.state.isMenuOpen && this.props.isBulkEditMode) {
      this.toggleMenu();
    }
  }
  ...
  render() {
    ...
    const isCloneDisabled =
      ...
      (isExpense && currentReportType.useCashAdvance) ||
      isBulkEditMode;
    const isDiscardRequestDisabled =
    ...
    if (isNewPreRequest) {
      // disable button if request report is not FA-approved
      isSaveDisabled =
        expReport.status !== status.ACCOUNTING_AUTHORIZED &&
        expReport.status !== status.FULLY_PAID;
    }
    ...
    const renderCreateReportOrSaveButton = () => {
      const disabledButtonClass = isSaveDisabled
        ? `${ROOT}__disabled-button`
        : '';

      const button = (
        <Button
          className={`${ROOT}__save ${disabledButtonClass} ${preRequestClass}`}
      ...
      if (isNewPreRequest && isSaveDisabled) {
        return (
          <Tooltip
            align="top right"
            className={`${ROOT}__tooltip`}
            content={msg().Exp_Msg_AccountingProcessInProgress}
          >
            <div className={`${ROOT}__tooltip-content`}>{button}</div>
          </Tooltip>
        );
      }
      return button;
    ...
    return (
      <div className={ROOT}>
        <div className={`${ROOT}__main`}>
          ...
          {renderCreateReportOrSaveButton()}
            ...
            {(!isNewPreRequest && (
              <>
                <button
                  ...
                  disabled={
                    !(this.props.readOnly && isCancellable) || isBulkEditMode
                  }
                >
                  {msg().Exp_Lbl_Recall}
                </button>
                <button
                  ...
                  disabled={!this.props.expReport.requestId || isBulkEditMode}
                >
                  {msg().Com_Btn_ApprovalHistory}
                </button>
                <button
                ...
                {!this.props.isFinanceApproval && (
                  <button
                    ...
                    disabled={!hasReportId || isBulkEditMode}
                  >
              ...
              <button
                ...
                disabled={isDiscardRequestDisabled}
                onClick={(event: React.SyntheticEvent<HTMLButtonElement>) => {
                  event.preventDefault();
                  event.stopPropagation();
