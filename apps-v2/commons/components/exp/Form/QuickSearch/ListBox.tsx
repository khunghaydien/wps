...
interface Props {
  ...
  onSelect: (value: Option, selectData?: any) => void;
}
...
const ListBox = ({
  ...
  const isShowJctRegistrationNumber = (option: Option): boolean => {
    return (
      useJctRegistrationNumber &&
      (!!option.jctRegistrationNumber ||
        (!option.jctRegistrationNumber && option.isJctQualifiedInvoiceIssuer))
    );
  };
      ...
      {(noOptions && <S.EmptyList> {msg().Com_Lbl_NoOption}</S.EmptyList>) ||
        options.map((option: Option, index) => (
          <S.Option
            ...
            id={option.id}
            onClick={() => onSelect(option, option.selectData)}
          ...
          >
            ...
            {isShowJctRegistrationNumber(option) &&
