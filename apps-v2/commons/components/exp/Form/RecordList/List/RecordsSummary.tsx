...
const renderTaxArea = (
  ...
  const isItemized = isItemizedRecord(records[0].items.length);
  const groupByTaxType = _.groupBy(records, 'items.0.taxTypeName');

  const recordItem = records[0].items[0];
  const isMultipleTax = recordItem?.taxItems?.length > 0;

  if (isMultipleTax) {
    const totalTaxAmount = records.reduce(
      (acc, record) => acc + (record.items[0].gstVat || 0),
      0
    );

    return (
      <div className={`${ROOT}__tax`}>
