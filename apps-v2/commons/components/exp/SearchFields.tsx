...
const CHECKBOX = { CHECKBOX: 'CHECKBOX' } as const;
...
const SearchFields = (props: Props) => {
  ...
  const fields = props.filters.map((filter) => {
    switch (filter.type) {
      ...
      case FILTER_TYPE.CHECKBOX:
        return (
          <div className={`${ROOT}-${filter.appendedClass}`}>
            <CheckBox checked={filter.checked} onChange={filter.onToggle}>
              {filter.label}
            </CheckBox>
          </div>
        );
  ...
  const searchBtn = (
    <Button
      ...
      type={props.isSearchBtnPrimary ? 'primary' : 'default'}
    >
