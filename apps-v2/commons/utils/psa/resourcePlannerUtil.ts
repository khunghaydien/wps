...
export const processView = (
  ...
  switch (viewType) {
    ...
    case ViewTypes.MONTH:
      ...
      sliceArray = Array.from(Array(12)).map((i, index) => {
        const day =
          index === 0
            ? moment(roleStartDate)
            : moment(roleStartDate).add(index, 'M');
        ...
        if (index === 0) {
          daysInMonth =
            moment(roleStartDate).daysInMonth() -
            +moment(roleStartDate).format('D') +
            1;
        }
        return daysInMonth;
      });
  ...
  let startIndex = pageNumber * sliceLength * viewLimit;
  let endIndex = startIndex + sliceLength * viewLimit;

  // for month view, next window should skip 365 days
  if (viewType === ViewTypes.MONTH) {
    startIndex = pageNumber * 366;
