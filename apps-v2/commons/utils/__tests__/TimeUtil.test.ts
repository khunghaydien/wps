...
describe('supportFormatFor3Digit(value) のテスト', () => {
  describe('1,2桁の数字の場合 e.g. 12 → 12:00', () => {
    test('1桁の数字が「時」として評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('1')).toBe('01:00');
    });
    test('2桁の数字が「時」として評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('12')).toBe('12:00');
    });
    test('2バイト文字の数字でも評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('１２')).toBe('12:00');
    });
    test('(マイナス)1桁の数字が「時」として評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('-1')).toBe('-01:00');
    });
    test('(マイナス)2桁の数字が「時」として評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('-12')).toBe('-12:00');
    });
    test('(マイナス)2バイト文字の数字でも評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('ー１２')).toBe('-12:00');
    });
  });

  describe('3〜4桁の数字の場合 e.g. 1234 → 12:34', () => {
    test('3桁目以上が「時」端数が「分」として評価されること（3桁）', () => {
      expect(TimeUtil.supportFormatFor3Digit('123')).toBe('01:23');
    });
    test('3桁目以上が「時」端数が「分」として評価されること（4桁）', () => {
      expect(TimeUtil.supportFormatFor3Digit('1234')).toBe('12:34');
    });
    test('「分」の部分が60以上の場合、「時」の桁上りに反映されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('1260')).toBe('13:00');
    });
    test('2バイト文字の数字でも評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('１２３４')).toBe('12:34');
    });
    test('(マイナス)3桁目以上が「時」端数が「分」として評価されること（3桁）', () => {
      expect(TimeUtil.supportFormatFor3Digit('-123')).toBe('-01:23');
    });
    test('(マイナス)3桁目以上が「時」端数が「分」として評価されること（4桁）', () => {
      expect(TimeUtil.supportFormatFor3Digit('-1234')).toBe('-12:34');
    });
    test('(マイナス)「分」の部分が60以上の場合、「時」の桁上りに反映されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('-1260')).toBe('-13:00');
    });
    test('(マイナス)2バイト文字の数字でも評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('ー１２３４')).toBe('-12:34');
    });
  });

  describe('5桁以上の数字の場合 e.g. 12345 → 123:45', () => {
    test('先頭4文字が評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('12345')).toBe('123:45');
    });
    test('先頭5文字が評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('123456')).toBe('1234:56');
    });
    test('(マイナス)先頭4文字が評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('-12345')).toBe('-123:45');
    });
    test('(マイナス)先頭5文字が評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('-123456')).toBe('-1234:56');
    });
  });
  ...
  describe('「nn:nn」形式の場合 e.g. 12:34 → 12:34', () => {
    test('HH:mm形式のまま出力されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('12:34')).toBe('12:34');
    });
    test('「分」が60以上であれば、「時」の桁上がりとして評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('12:120')).toBe('14:00');
    });
    test('2バイト文字でも評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('１２：３４５')).toBe('17:45');
    });
    test('引数の「分」が空文字列であれば、0分として解釈されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('12:')).toBe('12:00');
    });
    test('(マイナス)HH:mm形式のまま出力されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('-12:34')).toBe('-12:34');
    });
    test('(マイナス)「分」が60以上であれば、「時」の桁上がりとして評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('-12:120')).toBe('-14:00');
    });
    test('(マイナス)2バイト文字でも評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('ー１２：３４５')).toBe('-17:45');
    });
    test('(マイナス)引数の「分」が空文字列であれば、0分として解釈されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('-12:')).toBe('-12:00');
    });
    test('引数の「時」が空文字列は異常値であるため、空文字列が返却されること', () => {
      expect(TimeUtil.supportFormatFor3Digit(':30')).toBe('');
    });
  });

  describe('「nn.nn」形式の場合 e.g. 12.5 → 12:30', () => {
    test('小数部分が「分」の係数として評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('12.5')).toBe('12:30');
    });
    test('計算結果が四捨五入されること（.508 → 30.48分）', () => {
      expect(TimeUtil.supportFormatFor3Digit('12.508')).toBe('12:30');
    });
    test('計算結果が四捨五入されること（.509 → 30.54分）', () => {
      expect(TimeUtil.supportFormatFor3Digit('12.509')).toBe('12:31');
    });
    test('2バイト文字でも評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('１２．５')).toBe('12:30');
    });
    test('引数の「分」が空文字列であれば、0分として解釈されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('12.')).toBe('12:00');
    });
    test('(マイナス)小数部分が「分」の係数として評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('-12.5')).toBe('-12:30');
    });
    test('(マイナス)計算結果が四捨五入されること（.508 → 30.48分）', () => {
      expect(TimeUtil.supportFormatFor3Digit('-12.508')).toBe('-12:30');
    });
    test('(マイナス)計算結果が四捨五入されること（.509 → 30.54分）', () => {
      expect(TimeUtil.supportFormatFor3Digit('-12.509')).toBe('-12:31');
    });
    test('(マイナス)2バイト文字でも評価されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('ー１２．５')).toBe('-12:30');
    });
    test('(マイナス)引数の「分」が空文字列であれば、0分として解釈されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('-12.')).toBe('-12:00');
    });
    test('引数の「時」が空文字列は異常値であるため、空文字列が返却されること', () => {
      expect(TimeUtil.supportFormatFor3Digit('.5')).toBe('');
    });
  });

  describe('数値の0に該当し「00:00」として評価されるべき値', () => {
    test('0', () => {
      expect(TimeUtil.supportFormatFor3Digit('0')).toBe('00:00');
    });
    test('000', () => {
      expect(TimeUtil.supportFormatFor3Digit('000')).toBe('00:00');
    });
    test('00000', () => {
      expect(TimeUtil.supportFormatFor3Digit('00000')).toBe('00:00');
    });
    test('０００００', () => {
      expect(TimeUtil.supportFormatFor3Digit('０００００')).toBe('00:00');
    });
    test('0.00', () => {
      expect(TimeUtil.supportFormatFor3Digit('0.00')).toBe('00:00');
    });
    test('-0', () => {
      expect(TimeUtil.supportFormatFor3Digit('-0')).toBe('00:00');
    });
    ...
    test('ー０００００', () => {
      expect(TimeUtil.supportFormatFor3Digit('ー０００００')).toBe('00:00');
    });
    test('-0.00', () => {
      expect(TimeUtil.supportFormatFor3Digit('-0.00')).toBe('00:00');
    });
  ...
  describe('最大値以上の値は対象外なので、空文字列が返却される', () => {
    describe('1,2桁の数字', () => {
      test('対応内・プラス値で最大値以下', () => {
        expect(TimeUtil.supportFormatFor3Digit('47', 47 * 60)).toBe('47:00');
      });
      test('対応外・プラス値で最大値より大きい', () => {
        expect(TimeUtil.supportFormatFor3Digit('48', 48 * 60 - 1)).toBe('');
      });
      test('対応内・マイナス値で最小値以上', () => {
        expect(TimeUtil.supportFormatFor3Digit('-47', 47 * 60)).toBe('-47:00');
      });
      test('対応外・マイナス値で最小値より小さい', () => {
        expect(TimeUtil.supportFormatFor3Digit('-48', 48 * 60 - 1)).toBe('');
      });
    });
    describe('3,4桁の数字', () => {
      test('対応内・プラス値で最大値以下', () => {
        expect(TimeUtil.supportFormatFor3Digit('4800', 48 * 60)).toBe('48:00');
      });
      test('対応外・プラス値で最大値より大きい', () => {
        expect(TimeUtil.supportFormatFor3Digit('4801', 48 * 60)).toBe('');
      });
      test('対応内・マイナス値で最小値以上', () => {
        expect(TimeUtil.supportFormatFor3Digit('-4800', 48 * 60)).toBe(
          '-48:00'
        );
      });
      test('対応外・マイナス値で最小値より小さい', () => {
    ...
    describe('5桁以上の数', () => {
      test('対応内・プラス値で最大値以下', () => {
        expect(TimeUtil.supportFormatFor3Digit('12345', 123 * 60 + 45)).toBe(
          '123:45'
        );
      });
      test('対応外・プラス値で最大値より大きい', () => {
        expect(TimeUtil.supportFormatFor3Digit('12346', 123 * 60 + 45)).toBe(
          ''
        );
      });
      test('対応内・マイナス値で最小値以上', () => {
    ...
    describe('「nn:nn」形式', () => {
      test('対応内・プラス値で最大値以下', () => {
        expect(TimeUtil.supportFormatFor3Digit('46:119', 48 * 60 - 1)).toBe(
          '47:59'
        );
      });
      test('対応外・プラス値で最大値より大きい', () => {
        expect(TimeUtil.supportFormatFor3Digit('46:120', 48 * 60 - 1)).toBe('');
      });
      test('対応内・マイナス値で最小値以上', () => {
        expect(TimeUtil.supportFormatFor3Digit('-46:119', 48 * 60 - 1)).toBe(
          '-47:59'
    ...
    describe('「nn.nn」形式', () => {
      test('対応内・プラス値で最大値以下', () => {
        expect(TimeUtil.supportFormatFor3Digit('47.991', 48 * 60 - 1)).toBe(
          '47:59'
        );
      });
      test('対応外・プラス値で最大値より大きい', () => {
        expect(TimeUtil.supportFormatFor3Digit('47.992', 48 * 60 - 1)).toBe('');
      });
      test('対応内・マイナス値で最小値以上', () => {
        expect(TimeUtil.supportFormatFor3Digit('-47.991', 48 * 60 - 1)).toBe(
          '-47:59'
  ...
  describe('その他の異常値', () => {
    test('""', () => {
      expect(TimeUtil.supportFormatFor3Digit('')).toBe('');
    });
    test('":"', () => {
      expect(TimeUtil.supportFormatFor3Digit(':')).toBe('');
    });
    ...
    test('"a:00"', () => {
      expect(TimeUtil.supportFormatFor3Digit('a:00')).toBe('');
    });
    test('"あ:00"', () => {
      expect(TimeUtil.supportFormatFor3Digit('あ:00')).toBe('');
    });
    test('"安:00"', () => {
      expect(TimeUtil.supportFormatFor3Digit('安:00')).toBe('');
    });
    test('"11:1a"', () => {
      expect(TimeUtil.supportFormatFor3Digit('11:1a')).toBe('');
    });
  });
});
...
describe('マイナス分の時間変換のテスト', () => {
  describe('toHmm', () => {
    test('分1桁', () => {
      expect(TimeUtil.toHmm(-425)).toBe('-7:05');
    });
    test('分2桁', () => {
      expect(TimeUtil.toHmm(-460)).toBe('-7:40');
    });
    test('分のみ', () => {
      expect(TimeUtil.toHmm(-10)).toBe('-0:10');
    });
  });
  describe('toHHmm', () => {
    test('分1桁', () => {
      expect(TimeUtil.toHHmm(-425)).toBe('-07:05');
    });
    test('分2桁', () => {
      expect(TimeUtil.toHHmm(-460)).toBe('-07:40');
    });
    test('分のみ', () => {
      expect(TimeUtil.toHHmm(-10)).toBe('-00:10');
    });
  });
  describe('formatMinutesToHmm', () => {
    test('分1桁', () => {
      expect(TimeUtil.formatMinutesToHmm(-425)).toBe('-7 : 05');
    });
    test('分2桁', () => {
      expect(TimeUtil.formatMinutesToHmm(-460)).toBe('-7 : 40');
    });
    test('分のみ', () => {
      expect(TimeUtil.formatMinutesToHmm(-10)).toBe('-0 : 10');
    });
    test('分なし', () => {
      expect(TimeUtil.formatMinutesToHmm(-420)).toBe('-7');
    });
  });
  describe('toHours', () => {
    test('toHours', () => {
      expect(TimeUtil.toHours(-460)).toBe(-7);
    });
  });
});

describe('toHmmObj()のテスト', () => {
  test('プラス値', () => {
    expect(TimeUtil.toHmmObj(460)).toEqual({ h: '7', m: '40' });
  });
  test('マイナス値(非対応なので不正な値となる)', () => {
    expect(TimeUtil.toHmmObj(-460)).toEqual({ h: '-8', m: '40' });
  });
});
