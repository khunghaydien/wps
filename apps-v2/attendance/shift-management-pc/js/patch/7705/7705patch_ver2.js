...
teasp.view.EmpWorks.prototype.openExpApply = function(){
	...
	if(!works.length
	&& !confirm(teasp.message.getLabel('tf10008160', // 工数実績が入力されていません。{0}してよろしいですか？
...
teasp.view.EmpWorks.prototype.checkPreApplyMonthly = function(dlst){
	...
	if(this.pouch.isCheckDailyFixLeak() && noDailyFixed.length > 0){
		alert(wtitle + '\n' + teasp.message.getLabel('tm10001710', noDailyFixed.join(',')));
		return false;
	}
	return true;
};
...
teasp.dialog.InputTime.prototype.isReadOnly = function(){
	if(this.pouch.isJobMonthFixedByDate(this.args.date)){
		return true;
	}
	return (this.pouch.isEmpMonthReadOnly() || this.dayWrap.isDailyFix() || this.readOnly || !this.pouch.isUpdater());
};
