 ...
 * @author DCI小島
 */
teasp.view.Ext = function(){
    this.queryObj = null;
};

teasp.view.Ext.prototype = new teasp.view.Base();

teasp.view.Ext.prototype.contact = function(req, onSuccess, nowait){
 ...
 */
teasp.view.Ext.prototype.init = function(onSuccess, onFailure){
    globalLoadRes.today = new Date();
    globalLoadRes.todayYm = globalLoadRes.today.getFullYear() * 100 + (globalLoadRes.today.getMonth() + 1);
    var admin = (globalLoadRes.user[0].Profile.PermissionsModifyAllData || (globalLoadRes.emp.length && globalLoadRes.emp[0].IsAdmin__c));

    if(admin){
        if(!tsq.initialContents(this.errorAreaId, function(){ location.reload(); })){
            return;
        }
    }

    var select = dojo.byId('querySelect');
    dojo.create('option', { innerHTML: '', value: '-' }, select);
    var qlst = [];
    var defq = null;
    for(var i = 0 ; i < globalLoadRes.querys.length ; i++){
        var q = globalLoadRes.querys[i];
        if((!admin && !q.Permit__c) || q.Removed__c){
            continue;
        }
        qlst.push(q.Id);
        if(!defq){
        ...
        dojo.create('option', { innerHTML: q.Name, value: q.Id }, select);
    }
    if(qlst.length != 1){
        defq = null;
    }
    ...
    if(defq){
        select.value = defq.Id;
        dojo.byId('queryDescription').innerHTML = (defq.Description__c || '');
    }
    dojo.connect(select, 'onchange', this, function(){
        this.clearForm();
        var queryId = dojo.byId('querySelect').value;
        if(queryId == '-' || queryId == '*'){
            dojo.byId('queryDescription').innerHTML = '';
        }else{
            for(var i = 0 ; i < globalLoadRes.querys.length ; i++){
                var q = globalLoadRes.querys[i];
                if(q.Id == queryId){
                    dojo.byId('queryDescription').innerHTML = (q.Description__c || '');
                }
            }
        }
    });
    dojo.connect(dojo.byId('queryStart'), 'onclick', this, this.queryStep1);
};

/**
 * クエリ選択→サーバへ選択したクエリの情報要求
 *
 */
teasp.view.Ext.prototype.queryStep1 = function(){
    var select = dojo.byId('querySelect');
    ...
    if(queryId == '-'){
        ...
        tsq.QueryObj.prototype.buildForm = tsq.QueryObj.prototype.buildForm0;
        tsq.QueryObj.prototype.buildData = tsq.QueryObj.prototype.buildData0;
        this.initQuery();
    }
};
 ...
 *
 */
teasp.view.Ext.prototype.queryStep2 = function(result){
    document.body.style.cursor = 'default';
    console.log(result);
    this.initQuery(result.ext[0]);
};

teasp.view.Ext.prototype.initQuery = function(q){
    this.clearForm();
    if(this.queryObj){
        this.queryObj.destroy();
    }
    this.queryObj = new tsq.QueryObj(q);
    this.queryObj.buildForm();
};

teasp.view.Ext.prototype.clearForm = function(){
    var form = dojo.byId('queryForm');
    while(form.firstChild){
        dojo.destroy(form.firstChild);
    }
};
...
//------------------------------------------------------------------------------
var tsq = {};
tsq.initialContents = function(errorAreaId, onSuccess) {
    var DAILY_DATA = '日次データ出力';
    var reqs = [];
    var contents = [
        { name: DAILY_DATA   , revision: 4, permit: 0 }
    ];
    var entries = [];
    var delm = {};
    var emap = {};
    var orderNum = globalLoadRes.querys.length + 1;
    if(globalLoadRes.querys.length <= 0){
        entries = contents;
    }else{
        for(var i = 0 ; i < globalLoadRes.querys.length ; i++){
            emap[globalLoadRes.querys[i].Name] = 1;
            if(globalLoadRes.querys[i].Removed__c){
                delm[globalLoadRes.querys[i].Name] = 1;
            }
            for(var c = 0 ; c < contents.length ; c++){
                if(globalLoadRes.querys[i].Name == contents[c].name
                && !globalLoadRes.querys[i].Removed__c
                && (globalLoadRes.querys[i].Revision__c || 0) < contents[c].revision){
                    contents[c].permit = globalLoadRes.querys[i].Permit__c;
                    contents[c].order = globalLoadRes.querys[i].Order__c;
                    entries.push(contents[c]);
                }
            }
        }
        for(var c = 0 ; c < contents.length ; c++){
            if(!emap[contents[c].name] && !delm[contents[c].name]){
                entries.push(contents[c]);
            }
        }
    }
    for(var i = 0 ; i < entries.length ; i++){
        if(entries[i].name == DAILY_DATA){
            reqs.push({
                funcName: 'inputContent',
                params  : {
                    name        : DAILY_DATA,
                    ...
                    description : '日次データを出力します',
                    customized  : false,
                    revision    : entries[i].revision,
                    permit      : entries[i].permit,
                    order       : (typeof(entries[i].order) == 'number' ? entries[i].order : (orderNum++))
                }
            });
        }
    }
...
tsq.getPatternByDate = function(patterns, dt){
...
tsq.setApplyToDayObj = function(obj, key, a){
    if(!obj[key]){
        obj[key] = [];
    }
    obj[key].push(a);
...
tsq.getValidApplys = function(amap){
    var obj = {};
    for(var key in amap){
...
tsq.getApplyKey = function(a, dkey){
...
* @param {Object} obj
*/
tsq.QueryObj = function(obj){
    this.obj = obj;
    this.loadScript();
    this.dijitParts = {};
};

tsq.QueryObj.prototype.destroy = function(){
    for(var key in this.dijitParts){
        dijit.byId(key).destroyRecursive();
        delete this.dijitParts[key];
    }
};

tsq.QueryObj.prototype.contact = function(req, onSuccess, nowait){
    teasp.action.contact.remoteMethods(
        (is_array(req) ? req : [req]),
        {
            errorAreaId : (req.errorAreaId ? req.errorAreaId : this.errorAreaId),
...
tsq.QueryObj.prototype.loadScript = function(){
    this.errorAreaId = 'error_area';
    if(this.obj){
        dojo.eval(this.obj.Script1__c);
    }
};

tsq.QueryObj.prototype.getSoql1   = function(){ return this.obj.Soql1__c;   };
tsq.QueryObj.prototype.getSoql2   = function(){ return this.obj.Soql2__c;   };
tsq.QueryObj.prototype.getSoql3   = function(){ return this.obj.Soql3__c;   };
tsq.QueryObj.prototype.getScript1 = function(){ return this.obj.Script1__c; };

/**
* 検索を実行して、結果を取得、データ処理メソッドへ渡す
*
        ...
        that.contact(
            {
                ...
                params  : { soql: soql, limit: max, offset: offset }
            },
            function(result){
...
tsq.QueryObj.prototype.getCount = function(soql, nowait){
    var that = this;
    that.contact(
        {
...
* @param {string} value
*/
tsq.QueryObj.prototype.inputDownload = function(head, value, fname){
  ...
  this.contact(
      ...
      function(result, index){
          if(reqs.length <= (index + 1)){
              ...
              setTimeout(dojo.hitch(this, function(){ this.downloadDone(); }), 100);
          }
      },
      true
  );
...
tsq.QueryObj.prototype.step = function(){
    console.log('step');
};

tsq.QueryObj.prototype.downloadDone = function(){
    teasp.manager.dialogClose('BusyWait');
};

tsq.QueryObj.prototype.buildForm0 = function(){
    var area = dojo.byId('queryForm');
    var tbody = dojo.create('tbody', null, dojo.create('table', null, area));
    var row = dojo.create('tr', null, tbody);
    dojo.create('div', { innerHTML: 'Script' }, dojo.create('td', null, row));
    dojo.create('textarea', { id: 'queryScript', style: 'width:500px;height:200px;' }, dojo.create('td', null, row));
    var inp = dojo.create('input', { type: 'button', value: '実行' }, area);
    dojo.connect(inp, 'onclick', this, function(){
        var v = dojo.byId('queryScript').value;
        var form = dojo.byId('queryForm');
        while(form.firstChild){
            dojo.destroy(form.firstChild);
        }
...
tsq.QueryObj.prototype.buildData0 = function(records){
    var heads = '';
    var value = dojo.toJson(records);
    this.inputDownload(heads, value);
};
