teasp.provide('teasp.dialog.RequestList');
/**
 * 社員情報変更申請一覧ダイアログ
 *
 * @constructor
 ...
 */
teasp.dialog.RequestList = function(){
	this.widthHint = 588;
	this.heightHint = 334;
	this.id = 'dialogRequestList';
...
teasp.dialog.RequestList.prototype = new teasp.dialog.Base();

/**
 * @override
 */
teasp.dialog.RequestList.prototype.preInit = function(){
 ...
 * @override
 */
teasp.dialog.RequestList.prototype.ready = function(){
 ...
 * @override
 */
teasp.dialog.RequestList.prototype.preStart = function(){
 ...
 * @override
 */
teasp.dialog.RequestList.prototype.postShow = function(){
	// 一覧の内容をクリア
 ...
 * 申請一覧を取得取得
 */
teasp.dialog.RequestList.prototype.getRequestList = function() {
 ...
 * 申請一覧表示
 */
teasp.dialog.RequestList.prototype.createRequestList = function(lst) {
	this.requests = lst;
	var tbody = dojo.byId( 'RequestListTable_Body' );
		...
		for (var j = 0 ; j < this.colList.length ; j++ ) {
			...
			if (requestObj && requestObj[colObj.field]) {
				...
				case "name":
					...
					this.eventHandles.push(dojo.connect( aObj, "onclick", this, this.openRequestEdit( requestObj["id"] ) ));
					break;
				case "applyDatetime":
 ...
 * 申請編集画面表示
 */
teasp.dialog.RequestList.prototype.openRequestEdit = function( requestId ) {
	return function() {
		var req = {
