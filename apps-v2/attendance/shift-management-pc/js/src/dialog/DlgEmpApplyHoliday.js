...
teasp.dialog.EmpApply.prototype.createHolidayForm = function(key, node, contId, applyObj, btnbox){
	...
	if(applyObj && (fix || !applyObj.active)){
		...
		dojo.create('select', { id:'dialogApplyHolidayList' + contId, className:inputClass, style:'margin:0px 2px;width:100%;' }, dojo.create('td', null, cr));
	}
	...
	var changedTime = function(){
		...
		if(o.range == teasp.constant.RANGE_TIME){ // 時間単位有休
			...
			if(o.yuqSpend){
				...
				t = Math.ceil(t / 30) * 30;
				dojo.byId('dialogApplyTimeNote' + contId).innerHTML = teasp.message.getLabel('tm10003135', o.manageName, teasp.util.time.timeValue(t));
			}
	...
	var changedHolidayType = function(o, flag, applyObj){
		...
		this.disableApplyButton(contId, false);

		if(stockable && o.range == teasp.constant.RANGE_TIME && this.pouch.getBaseTimeForStock() == null){
			this.disableApplyButton(contId, true);
			teasp.dialog.EmpApply.showError(contId, teasp.message.getLabel('tm10011090'), true); // 勤務体系の『時間単位休の基準時間（日数管理休暇用）』が未設定のためこの休暇を利用できません
		}else if(stockable && !flag){ // 代休かつ代休管理する設定または日数管理する休暇を選択
			var anyZan;
			var isDisplayZan = true;
			if(o.type == teasp.constant.HOLIDAY_TYPE_DAIQ){
				anyZan = this.daiqZan;
			}else{
				anyZan = this.stockZan[o.manageName];
				// 残日数が1日未満かつ将来基準時間変更が予定されている場合は取得可能日数を表示しない
				var spendTimeUnitFlag = teasp.data.Pouch.getSpendStockChangedBaseTime(
					anyZan,
					this.pouch.getConfigHistory(),
					this.args.date
				);
				if(spendTimeUnitFlag >= 2 && anyZan.zan < 1){
					isDisplayZan = false;
				}
			}
			if(isDisplayZan){
				var roundUp = anyZan.startDateMin
						? teasp.data.Pouch.isZanRoundUp(this.pouch.getConfigHistory(), anyZan, this.args.date)
						: false;
				dojo.byId('dialogApplyAnnotate' + contId).innerHTML
					...
					, teasp.data.Pouch.formatDays(anyZan.zan || 0, {
	...
	var setHolidaySelect = function(contId, applyObj){
		var select = dojo.byId('dialogApplyHolidayList' + contId);
		if(select){
			...
			for(var i = 0 ; i < this.holidayList.length ; i++){
				...
				if(h.managed
				&& !this.stockZan[h.manageName].valid
				&& (!applyObj || applyObj.holiday.id != h.id)){
					continue;
				}
	...
	teasp.data.Pouch.checkStockBaseTime(
		...
		this.args.date,
		null,
		dojo.hitch(this, function(flag, result, mapError){
			if(flag >= 1){
				this.pouch.setKeyObj('stocks', result);
				for(var i = 0 ; i < this.holidayList.length ; i++){
					...
					this.stockZan[holiday.manageName] = teasp.data.Pouch.getStockZan( // 当日取得可能な積休の残日数を取得
							...
							holiday.manageName,
							this.args.date);
					if(mapError && mapError[holiday.manageName]){
						// ※エラーが返されている。エラー表示せず残＝０として扱う
						this.stockZan[holiday.manageName].zan = 0;
						this.stockZan[holiday.manageName].valid = false;
					}else if(this.stockZan[holiday.manageName].zan <= 0
				...
				teasp.dialog.EmpApply.showError(contId, result);
			}
			setHolidaySelect.apply(this, [contId, applyObj]);
		})
	);
	...
	if(btnOk){
		this.eventHandles.push(dojo.connect(btnOk, 'onclick', this, function(e){
			...
			if(o.range == teasp.constant.RANGE_TIME){ // 時間単位有休
				if(this.pouch.isHourlyLeaveLimitOfDay(this.args.date)){
					teasp.dialog.EmpApply.showError(contId, teasp.message.getLabel('tf10010900'), true); // 1日に申請可能な時間単位休の上限に達しています。
					return;
				}
				...
				var rest2 = rests.free.concat(rests.paid).concat(rests.unpaid); // 任意休憩と時間単位休のミックス
				if(rest2.length > 0){
				...
				if(this.dayWrap.isConflictReviseTime(st, et, minUnit)){
					teasp.dialog.EmpApply.showError(contId, teasp.message.getLabel('tf10011215')); // 承認待ちの勤怠時刻修正申請の内容と重なるため申請できません
					return;
				}
				req.apply.startTime = st;
				req.apply.endTime   = et;
				if(manad){ // 日数管理休暇用に消化情報をセット
					manad.spd  = t / this.pouch.getBaseTime();
					manad.ds   = [req.apply.startDate];
				}

				//休暇時間の制限
				if(o.timeUnit){
					var unitMod = t % o.timeUnit;
					if( unitMod != 0 ){
						var unitH = Math.floor(t/60);
						var unitMtemp = Math.floor(t%60);
						var unitMM = (unitMtemp < 10 ? '0' : '' ) + unitMtemp;
						var unitUnit = (o.timeUnit != 60) ? teasp.message.getLabel('th00000100',o.timeUnit) : teasp.message.getLabel('th00000050');						

						teasp.dialog.EmpApply.showError(contId, teasp.message.getLabel('th00000010', unitH,unitMM,unitUnit)); // 休暇時間（休憩時間を含まない時間）が{0}:{1}です。\n休暇開始時間や休暇終了時間を調整して、{2}単位で申請してください。
						return;
					}
				}

			}else{ // 時間単位有休以外
				if(!this.dayWrap.isFlexHalfDayTimeDay() // コア時間帯が設定されていないフレックスの半日休暇適用時間を使用しない
				|| (o.range != teasp.constant.RANGE_AM && o.range != teasp.constant.RANGE_PM)){
					var ra = this.dayWrap.getEmpApplyByKey(teasp.constant.APPLY_KEY_REVISETIME); // 勤怠時刻修正申請
					if(ra && ra.status == teasp.constant.STATUS_WAIT){
						teasp.dialog.EmpApply.showError(contId, teasp.message.getLabel('tf10011300', teasp.util.date.formatDate(this.args.date, 'M/d'))); // {0}に承認待ちの勤怠時刻修正申請があるため申請できません。
						return;
					}
				}
			...
			if(!this.checkPluginHolidayApply(contId, req, o, spend, manad)){
				return;
			}
			var ngd = []; // 当月で残日数不足になる有休計画付与日を入れる変数
			var wad = []; // 次月以降の残日数不足になる有休計画付与日を入れる変数
			if(spend){ // 有休消化する休暇の場合、有休残日数のチェックをする
				var syr = this.pouch.simulateDspYuqRemain(req.apply.startDate, spend); // 当月以降～次回付与予定日までの有休計画付与日の情報
				...
				if(wad.length > 0){
					...
					teasp.manager.dialogOpen('MessageBox', {
						...
						message : teasp.message.getLabel('tk10005080', wad.join(', '))
					}, this.pouch, this, function(){
						this.submitHolidayApply(contId, req, o, manad);
					});
				}
			}
			if(!wad.length){
...
teasp.dialog.EmpApply.prototype.submitHolidayApply = function(contId, req, o, manad){
...
//				}
	...
	// 期間内の申請をチェック
	if(o.range != teasp.constant.RANGE_TIME){ // 時間単位有休以外
 ...
 * @param {string} contId IDに使用する番号
 * @param {Object} req 	申請パラメータ
 * @param {Object} o 休暇オブジェクト
 * @param {Object|null} spend 有休の消化情報
 * @param {Object|null} manad 日数管理休暇の消化情報
 * @returns {boolean}
 */
