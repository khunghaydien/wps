define([
	...
	"dojo/Deferred",
	"dojo/_base/lang",
	"dojo/text!tsext/check9544/Check9544View.html",
	"tsext/service/Request",
	"tsext/dialog/Confirm",
	"tsext/check9544/Emps",
	...
	return declare("tsext.check9544.Check9544View", [_WidgetBase, _TemplatedMixin], {
		...
		// 引数の"#" の後が "detail" なら詳細モード（例: /AtkConfigEditView?check=9544#!detail）
		isDetail: function(){
			return /^detail$/i.test(this.hash);
		},
		// デフォルトの検査対象の開始日付を得る
		getDefaultStartDt: function(){
			return moment().add(-2, 'y').format('YYYY-MM-DD'); // 本日日付の2年前の日付
		},
		//
		showExpert: function(param){
			query('div.check-expert', this.domNode).forEach(function(el){
				domStyle.set(el, 'display', (param.expert ? '' : 'none'));
			}, this);
			WaitProgress.showExpert(param);
		},
		// 社員を取得
		fetchEmps: function(){
			this.emps.fetch().then(
				lang.hitch(this, function(){
					...
					param.startDt = this.getDefaultStartDt();
					var args = Agent.getArgs();
					if(args.startdt){
						// 引数に"startdt=yyyymmdd"が指定されていたら（例：/AtkConfigEditView?check=9544&startDt=20190101#!check）
						// その日付を含む月度以降を診断する。省略時は現在日-2年
						var d = Util.parseDate(args.startdt);
						if(d){
							param.startDt = moment(d).format('YYYY-MM-DD');
						}
					}
					if(args.enddt){
						// 引数に"enddt=yyyymmdd"が指定されていたら（例：/AtkConfigEditView?check=9544&enddt=20190630#!check）
						// その日付を含む月度までを診断する。省略時はなし
						var d = Util.parseDate(args.enddt);
						if(d){
							param.endDt = moment(d).format('YYYY-MM-DD');
						}
					}
					if(args.hasOwnProperty('unsettled')){
						// 引数に"unsettled"が指定されていたら（例：/AtkConfigEditView?check=9544&unsettled#!check）
						// 未確定の月度も診断対象に含める
						param.unsettled = true;
					}
					if(args.hasOwnProperty('notbug')){
						// 引数に"notbug"が指定されていたら（例：/AtkConfigEditView?check=9544&unsettled#!check）
						// 診断対象となった月はすべて出力する
						param.notbug = true;
					}
					if(args.hasOwnProperty('expert')){
						// 引数に"expert"が指定されていたら（例：/AtkConfigEditView?check=9544&expert#!check）
						// 不具合件数の表示と不具合影響リスト.csvのダウンロードができる
						param.expert = true;
					}
					if(args.hasOwnProperty('notsqueeze')){
						// 引数に"notsqueeze"が指定されていたら（例：/AtkConfigEditView?check=9544&notsqueeze#!check）
						// 絞り込み条件を緩和する
						param.notSqueeze = true;
					}
					this.showExpert(param);
		...
		fetchEmpMonths: function(param){
			this.emps.fetchEmpMonths(param).then(
				lang.hitch(this, function(param){
					this.fetchEmpShifts(param);
				}),
				lang.hitch(this, this.showError)
			);
		},
		// シフト設定（日タイプ変更の勤務時間変更申請）を取得
		fetchEmpShifts: function(param){
			var shiftEmpIds = this.emps.get9393EmpIds();
			if(shiftEmpIds.length){
				this.emps.fetchEmpShifts(param, shiftEmpIds).then(
					lang.hitch(this, function(param){
		...
		loadEmpMonthPrint: function(param){
			this.emps.squeezeMonths(param.notSqueeze);
