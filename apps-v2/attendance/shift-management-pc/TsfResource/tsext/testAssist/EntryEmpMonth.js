define([
	...
	// 打刻
	return declare("tsext.testAssist.EntryEmpMonth", EntryBase1, {
		/**
		 * @constructor
		...
		constructor : function(args, lineNo){
			this.yearMonth = null;
			try {
				this.yearMonth = this.getYm(this.getItem(1));
			}catch(e){
		...
		execute: function(bagged){
			...
			var chain = Current.loadEmpMonth(req).then(
				...
				lang.hitch(this, function(errmsg){
					return bagged.stayResult(this.addError(Constant.ERROR_LEVEL_2, errmsg));
				})
			);
			chain.then(lang.hitch(this, function(bagged){
				...
				Current.loadEmpMonthDelay(lang.hitch(this, function(flag, errmsg){
					if(flag){
						...
						bagged.doneResult(this.addError(Constant.ERROR_LEVEL_2, errmsg));
					}
				}));
			}));
		}
