define([
	...
	// シフト振替申請
	return declare("tsext.testAssist.EntryApplyShiftChange", EntryBase2, {
		/**
		 * @constructor
		...
		lastCheck: function(){
			...
			var sd = this.getStartDate();
			var ed = this.getEndDate();
			// TODO シフト振替申請ができる条件を満たさない場合はエラーにする
			// @see teasp.data.Pouch.prototype.getShiftChangeableDates
/*
...
*/
		...
		getApplyEmpDayRequest: function(){
			var month = Current.getMonth();
			var apply = Current.getCanceldApply(Constant.APPLY_TYPE_SHIFTCHANGE, this.getStartDate());
			var sd = this.getStartDate();
			var ed = this.getEndDate();
			return {
				...
				apply: {
					...
					applyType		: Constant.APPLY_TYPE_SHIFTCHANGE,
					startDate		: sd,
					endDate			: sd,
					exchangeDate	: ed,
					note			: this.getNote(),
