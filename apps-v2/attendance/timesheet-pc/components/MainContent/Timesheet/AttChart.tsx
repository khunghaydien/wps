...
export default class AttChart extends React.Component<Props> {
  ...
  renderLeaveOfAbsence() {
    const { isLeaveOfAbsence, isAllowWorkDuringLeaveOfAbsence } = this.props;

    if (!isLeaveOfAbsence || isAllowWorkDuringLeaveOfAbsence) {
      return null;
    }
  ...
  render() {
    const { dayType, isHolLegalHoliday } = this.props;
