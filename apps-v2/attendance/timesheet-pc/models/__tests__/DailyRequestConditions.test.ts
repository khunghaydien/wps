...
import { STATUS } from '../../../domain/models/AttDailyRequest/BaseAttDailyRequest';
...
describe('timesheet-pc/models/DailyRequestConditions', () => {
      ...
      test('effectualAllDayLeaveTypeフィールドに、休暇タイプが適用されること', () => {
        const sampleDailyRequestConditions =
          DailyRequestConditions.createFromParams(
            ...
            {
              [DUMMY_REQUEST_ID_1]: LeaveRequest.convertFromBase({
      ...
      const sampleDailyRequestConditions =
        DailyRequestConditions.createFromParams(
          ...
          }, // AttRecord
          {
            ...
            [DUMMY_REQUEST_ID_2]: LeaveRequest.convertFromBase({
              ...BASE_APPROVED_REQUEST_PARAM,
              leaveRange: 'PM',
            }),
          }, // requestMap
  ...
  describe('collectRemarksFromRequests()', () => {
    describe('提出済みの申請から備考の内容を抽出する', () => {
      const sampleRequestWithRemarks1 = LeaveRequest.convertFromBase({
        requestTypeName: '申請種別1',
        remarks: '申請コメント1',
      });

      const sampleRequestWithRemarks2 = LeaveRequest.convertFromBase({
        requestTypeName: '申請種別2',
        remarks: '申請コメント2',
      });

      const sampleRequestWithEmptyStringRemarks = LeaveRequest.convertFromBase({
        requestTypeName: '申請種別3',
        remarks: '',
      });

      const sampleRequestWithoutRemarks = LeaveRequest.convertFromBase({
  ...
  describe('isAllowWorkDuringLeaveOfAbsence', () => {
    const BASE_ATTRECORD_ON_WORKDAY = {
      recordDate: '2018-05-19',
      dayType: AttRecord.DAY_TYPE.WORKDAY,
      hasActualWorkingTimes: false,
    ...
    test.each`
      ...
      ${STATUS.APPROVAL_IN}   | ${false}
      ${STATUS.APPROVED}      | ${true}
      ${STATUS.CANCELED}      | ${false}
      ${STATUS.REAPPLYING}    | ${false}
    `('休職休業中の就業を可能にする', ({ status, expected }) => {
      const someDailyRequestConditions =
        DailyRequestConditions.createFromParams(
          {
            ...BASE_ATTRECORD_ON_WORKDAY,
          ...
          {
            pattern: { ...BASE_DIRECT_REQUEST, status },
          }, // requestMap
          {}, // requestTypeMap
          {} // otherConditions
        );
      const result = someDailyRequestConditions.isAllowWorkDuringLeaveOfAbsence;
