...
it('should do.', async () => {
  ...
  expect(FixDailyRequestRepository.cancelApproval).toBeCalledTimes(1);
  expect(FixDailyRequestRepository.cancelApproval).toBeCalledWith('requestId');
});
...
it("should not execute if user's answer is false .", async () => {
  ...
  expect(FixDailyRequestRepository.cancelApproval).toBeCalledTimes(0);
});
