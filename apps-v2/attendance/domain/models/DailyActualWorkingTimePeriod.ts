...
export const buildDailyActualWorkingPeriodListFromAttRecord = (
  ...
  // 休憩時間
  attRecord.dailyRestList?.forEach(({ restStartTime, restEndTime }) => {
    const startTime =
      restStartTime === 0 ? restStartTime : restStartTime || null;
    const endTime = restEndTime === 0 ? restEndTime : restEndTime || null;
