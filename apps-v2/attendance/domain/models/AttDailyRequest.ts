...
import { STATUS, Status } from './AttDailyRequest/BaseAttDailyRequest';
...
export type {
  BaseAttDailyRequest,
  Status,
} from './AttDailyRequest/BaseAttDailyRequest';
...
/**
 * いずれ repository に移動する。
 * RemoteModel から DomainModel への変換を DomainModel で行っているため依存関係が発生してしまっている。
 * Repository に移動するときは各 RemoteXXX モデルを作り、DomainModel に convert させる。
 * 例： RemoteLeave.convert = (remote: RemoteLeave) : DomainLeave => {}
 * @param nameMap
 * @param fromRemote
 * @param timesheet
 * @returns
...
export const createFromRemote = (
  ...
  const request = Base.createFromRemote(nameMap, fromRemote, timesheet);

  switch (request.requestTypeCode) {
    case REQUEST_TYPE_CODE.Leave:
      return Leave.convertFromBase(request);
    case REQUEST_TYPE_CODE.HolidayWork:
      return HolidayWork.convertFromBase(request);
    ...
    case REQUEST_TYPE_CODE.EarlyLeave: {
      const dailyRecord = timesheet.records.find(
        ({ recordDate }) => recordDate === fromRemote.startDate
      );
      return {
        ...EarlyLeave.create({ ...request }),
        isFlexWithoutCore: !!dailyRecord?.isFlexWithoutCore,
      };
    }
...
export const createFromDefaultValue = (
  ...
  switch (requestTypeCode) {
    case REQUEST_TYPE_CODE.Leave:
      return Leave.convertFromBase(defaultValue);
    case REQUEST_TYPE_CODE.HolidayWork:
      return HolidayWork.convertFromBase(defaultValue);
    case REQUEST_TYPE_CODE.EarlyStartWork:
...
export const getAvailableRequestTypesAt = <U extends Timesheet>(
  ...
  timesheet: U
): DailyRequestNameMap | { code: string } => {
...
export const getEffectualLeaveRequests = (
  ...
  requests
    ...
    .map((request) => Leave.convertFromBase(request));
 ...
 */
export const isStatusChangeWorkingRule = (status: Status): boolean =>
