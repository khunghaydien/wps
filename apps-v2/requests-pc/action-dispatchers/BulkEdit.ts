...
import { actions as recordUpdatedActions } from '@apps/requests-pc/modules/ui/expenses/dialog/recordUpdated/dialog';
...
} from '@apps/requests-pc/action-dispatchers/ExpenseType';

/** update and clear record info on expense type change */
export const updateRecordInfo =
  ...
  async (dispatch: AppDispatch, getState: () => State) => {
    ...
    /** set default payment method when creating new record or switching expense type */
    ...
    switch (recordType) {
      case RECORD_TYPE.Mileage:
        ...
        if (isSameRecordType) {
          ...
          newExpRecord.mileageRouteInfo = mileageRouteInfo;
          ...
          newAmount = 0;
          // no mileage route info, initialise with 2 input fields
          newExpRecord.mileageRouteInfo = {
...
export const getMileageRateInfo =
  ...
  async (dispatch: AppDispatch): Promise<MileageRateInfo> => {
    const mileageRateList = await dispatch(
