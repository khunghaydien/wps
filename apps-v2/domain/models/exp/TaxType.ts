...
/**
 * Filter child tax type option list based on parent tax type list
 * Single Tax: only contains tax type selected in parent
 * Multiple Tax: only contains child tax types that belong in parent tax type list
 * @param childTaxTypeList
 * @param parentTaxTypeBaseId
 * @param parentTaxItemList
 * @returns ExpTaxTypeList
 */
export const getTaxTypeList = (
  childTaxTypeList: ExpTaxTypeList,
  parentTaxTypeBaseId: string,
  parentTaxItemList: TaxItems
) => {
  const isMultipleTax = parentTaxItemList?.length > 0;
  const parentTaxTypeIdList = (parentTaxItemList || []).map(
    ({ taxTypeBaseId }) => taxTypeBaseId
  );
  return childTaxTypeList.filter((childTaxType: ExpTaxType) => {
    return isMultipleTax
      ? parentTaxTypeIdList.includes(childTaxType.baseId)
      : childTaxType.baseId === parentTaxTypeBaseId;
  });
};
