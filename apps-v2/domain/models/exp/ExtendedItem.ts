...
export const totalNumExtendedItems = 10;
export const totalNumExpTypeExtendedItems = 5;
 ...
 */
export const initialEmptyEIs = (
  isExpenseTypeEI?: boolean
): ExtendedItemExpectedList => {
  const totalNumEI = isExpenseTypeEI
    ? totalNumExpTypeExtendedItems
    : totalNumExtendedItems;
  const eIValues = {};
  for (let i = 0; i < totalNumEI; i++) {
...
const existExtendedItems = (
  ...
  getExtendedItemArray(items, isExpenseTypeEI).filter(
    (i: ExtendedItem) => i.id
  );

const extendedItemToLabelValue = (
  extendedItem: ExtendedItem
): { id?: string; code?: string; label: string; value: string } => {
  if (!extendedItem.info) {
    return { label: '', value: '' };
  }
  let displayValue = '';
  let displayCode;
  const displayId = extendedItem.id;
...
export const getLabelValueFromEIs = (
  items: ExtendedItemExpectedList,
  isExpenseTypeEI?: boolean
): Array<{ id?: string; code?: string; label: string; value: string }> =>
  sortEIByType(existExtendedItems(items, isExpenseTypeEI)).map(
    (extendedItem: ExtendedItem) => extendedItemToLabelValue(extendedItem)
  );
/**
