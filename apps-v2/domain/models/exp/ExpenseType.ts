...
export type ExpenseType = ExtendedItemExpectedList & {
  ...
  allowSameExpType?: boolean;
  code: string;
  description: string;
  displayJctNumberInput?: boolean;
  displayMultipleTaxEntryForm?: boolean;
  expMileageRateId?: string;
  expTypeChildIds?: string[];
  ...
  isFavorite?: boolean;
  isGroup: boolean;
  itemizationSetting: ItemizationSetting;
...
export const buildExpTypeOptionList = (expTypeList: ExpenseTypeList) => {
  if (!expTypeList) return [];

  const placeholder = { value: '', text: msg().Exp_Lbl_PleaseSelect };
  const optionList = expTypeList.map(({ id, name }) => ({
    value: id,
    text: name,
  }));
...
export const defaultValue = {
  ...
  parentGroupId: '',
  isGroup: false,
  itemizationSetting: ITEMIZATION_SETTING_TYPE.NotUsed,
  hasChildren: false,
  recordType: '',
  fileAttachment: '',
