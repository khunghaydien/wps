...
import { iOS } from '@commons/utils/DeviceUtil';
...
let pdfjsWorkerBlob, pdfjsWorkerBlobURL;
export const pdfLib = () => {
  if (!pdfjsWorkerBlobURL) {
    pdfjsWorkerBlob = new Blob([pdfjsWorker]);
    pdfjsWorkerBlobURL = window.URL.createObjectURL(pdfjsWorkerBlob);
...
export const RECEIPT_LIBRARY_MAX_SELECTION_COUNT = 10;
...
export const getMetadataFromPdf = async (file?, pdfDoc?: OcrPdfDoc) => {
  try {
    ...
    if (pdfDoc) {
      ...
      doc = await pdfLib().getDocument({ url: fileUrl }).promise;
    }
    const promises = [];
    ...
    await Promise.all(
      pages.map(async (page, pageIdx) => {
        ...
        ops.argsArray.every((arg, i) => {
          ...
          if (
            t === pdfLib().OPS.paintJpegXObject ||
            t === pdfLib().OPS.paintImageXObject
          ) {
            imgCount++;
            imgKey = arg[0];
            imgPageIdx = pageIdx;
    ...
    if (img.src) {
      ...
      imgMetadata = await generateMetadata(dataURL);
    } else if (img.data) {
      const canvas = document.createElement('canvas');
      canvas.width = img.width;
      canvas.height = img.height;
...
const fileViewUrl = (contentDocId: string) => {
  return contentDocId && `/lightning/r/ContentDocument/${contentDocId}/view`;
};

/*
 * Fix iOS require re-login
 * https://teamspiritdev.atlassian.net/browse/EXP-8591
 */
export const getFilePreviewMB = (contentDocId: string) => {
  return iOS ? fileViewUrl(contentDocId) : fileDownloadUrl(contentDocId);
};
...
export const saveMetadataList = (
  metaDataList: Array<FileMetadata>
): Promise<any> => {
  return Api.invoke({
    path: '/exp/file-meta/save-list',
    param: { metaDataList },
  }).then((res) => res);
};
