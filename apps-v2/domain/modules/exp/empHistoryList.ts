  ...
  */
    if (dateTriggerActions.includes(action.type)) {
      ...
      if (isCrossHistory) {
        ...
        const currCompanyId = get(currentState, 'userSetting.companyId');
        const subRoleId = get(currentState, 'ui.expenses.subrole.selectedRole');
        const currHistory = find(
          empHistoryGroupList,
          ({ id }) => id === subRoleId
        );
        const currSubRoleKey = get(currHistory, 'subRoleKey', null);
        ...
        const targetHistory = find(
          empHistoryGroupList,
          ({ validFrom, validTo, subRoleKey, companyId }) =>
            isProxyMode
        ...
        if (!isProxyMode) {
          const targetHistoryId = get(targetHistory, 'id', subRoleId);
          dispatch(setSelectedSubRole(targetHistoryId));
          await dispatch(getUserSettings(targetHistoryId));
        }
