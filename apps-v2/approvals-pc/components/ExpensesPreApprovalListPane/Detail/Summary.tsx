  ...
  const renderLayout = (layout) =>
    layout.map((row, idx) => (
      <li key={`row-${idx}`} className={`${ROOT}__list-detail-item`}>
        <HorizontalLayout>
          {row.map(({ label, value, extra }: Record<string, any>, idx) => (
        ...
        <ul className={`${ROOT}__list-detail ${ROOT}__cash-advance`}>
          {renderFields([
            ...
            },
            {
              ...
              value:
                DateUtil.formatYMD(expRequest.cashAdvanceRequestDate) || '',
            },
            {
              label: msg().Exp_Clbl_CashAdvanceRequestPurpose,
              value: expRequest.cashAdvanceRequestPurpose || '',
            },
          ])}
        </ul>
      )}
