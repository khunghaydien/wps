...
import { isMileageRecord } from '@apps/domain/models/exp/Record';
...
const DetailContainer = ({
  ...
  const { recordType, validDateFrom } = props.tmpEditRecord;
  ...
  const ExpenseTypeDetailActions = useMemo(
    () =>
      bindActionCreators(
        { ...expenseTypeDetailActions, searchMileageRate },
        dispatch
      ),
    [dispatch]
  );
  ...
  useEffect(() => {
    checkForMileageChanges();
  }, [recordType, validDateFrom]);
  ...
  const checkForMileageChanges = () => {
    if (isMileageRecord(recordType)) {
      const targetDate = props.tmpEditRecord.validDateFrom;
      ExpenseTypeDetailActions.searchMileageRate({
        companyId,
        targetDate,
      });
