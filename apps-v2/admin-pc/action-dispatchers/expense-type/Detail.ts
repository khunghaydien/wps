...
import { actions as BaseRecordActions } from '../../modules/expense-type/entities/baseRecord';
import { actions as DetailActions } from '../../modules/expense-type/ui/detail';
...
export const setClonedRecord =
  (baseRecord: RecordUtil.Record) => (dispatch: AppDispatch) => {
    // @ts-ignore
    dispatch(DetailActions.setBaseRecord(baseRecord));
  };
...
const fetchDetail =
  ...
  async (dispatch: Dispatch<any>): Promise<void> => {
    const result = await getExpenseTypeById(baseId);
    dispatch(BaseRecordActions.fetch(result[0]));
    dispatch(DetailActions.setBaseRecord(result[0]));
  };
...
export const showDetail =
  ...
  async (dispatch: Dispatch<any>): Promise<void> => {
    ...
    try {
      ...
      await dispatch(fetchDetail(baseId));
      dispatch(showDetailPane(true));
...
export const createRecord =
  ...
  async (dispatch: Dispatch<any>): Promise<boolean> => {
    ...
    try {
      ...
      await dispatch(createExpenseType(record));
...
export const updateBase =
  (
    companyId: string,
    originalBaseRecord: ExpenseType,
    editingBaseRecord: ExpenseType,
    baseValueGetter: (arg0: string) => any,
    functionTypeList: FunctionTypeList
  ) =>
  async (dispatch: Dispatch<any>): Promise<boolean> => {
    ...
    try {
      ...
      await dispatch(updateExpenseType(record));
...
export const removeBase =
  ...
  async (dispatch: Dispatch<any>): Promise<boolean> => {
    ...
    try {
      app.loadingStart();
      await dispatch(deleteExpenseType({ id: baseId }));
      dispatch(showDetailPane(false));
      dispatch(setModeBase(DETAIL_PANEL_MODE.VIEW));
      return true;
    } catch (err) {
...
export const cancelEditing =
  (baseRecord: ExpenseType) => (dispatch: Dispatch<any>) => {
    dispatch(DetailActions.setBaseRecord(baseRecord));
    dispatch(setModeBase(DETAIL_PANEL_MODE.VIEW));
  };

export const changeBaseRecordValue =
  (key: keyof ExpenseType, value: ExpenseType[keyof ExpenseType]) =>
