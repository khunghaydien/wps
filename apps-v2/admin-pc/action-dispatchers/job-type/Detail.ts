...
import { actions as BaseRecordActions } from '../../modules/job-type/entities/baseRecord';
import { actions as DetailActions } from '../../modules/job-type/ui/detail';

import {
  createJobType,
  deleteJobType,
  updateJobType,
} from '../../actions/jobType';
...
export const showDetail =
  (companyId: string, baseRecord: JobType) =>
...
export const createRecord =
  ...
  async (dispatch: Dispatch<any>): Promise<boolean> => {
    ...
    try {
      ...
      await dispatch(createJobType(record));
...
export const updateBase =
  (
    companyId: string,
    originalBaseRecord: JobType,
    editingBaseRecord: JobType,
    baseValueGetter: (arg0: string) => any,
    functionTypeList: FunctionTypeList
  ) =>
  async (dispatch: Dispatch<any>): Promise<boolean> => {
    ...
    try {
      ...
      await dispatch(updateJobType(record));
...
export const removeBase =
  ...
  async (dispatch: Dispatch<any>): Promise<boolean> => {
    ...
    try {
      app.loadingStart();
      await dispatch(deleteJobType({ id: baseId }));
      dispatch(showDetailPane(false));
      dispatch(setModeBase(DETAIL_PANEL_MODE.VIEW));
      return true;
    } catch (err) {
...
export const cancelEditing =
  (baseRecord: JobType) => (dispatch: Dispatch<any>) => {
    dispatch(DetailActions.setBaseRecord(baseRecord));
    dispatch(setModeBase(DETAIL_PANEL_MODE.VIEW));
  };

export const changeBaseRecordValue =
  (key: keyof JobType, value: JobType[keyof JobType]) =>
  (dispatch: Dispatch<any>) => {
    dispatch(DetailActions.setBaseRecordByKeyValue(key, value));
  };

export const linkWorkCategories =
  (jobTypeId: string, ids: string[]) => async (dispatch: Dispatch<any>) => {
    const app = App(dispatch);
    try {
      app.loadingStart();
      await Repository.linkWorkCategories(jobTypeId, ids);
      return true;
    } catch (err) {
      app.catchApiError(err);
    } finally {
...
export const unlinkWorkCategories =
  (jobTypeId: string, ids: string[]) => async (dispatch: Dispatch<any>) => {
    const app = App(dispatch);
    try {
      app.loadingStart();
      await Repository.unlinkWorkCategories(jobTypeId, ids);
      return true;
    } catch (err) {
      app.catchApiError(err);
    } finally {
