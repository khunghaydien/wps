...
} from '@attendance/repositories/__tests__/mocks/att-annual-leave-grant-list';
...
describe('admin-pc/annual-paid-leave-management/modules/detail-pane/entitites/grant-history-list', () => {
  ...
  describe('action', () => {
    describe('fetch ~ fetchSuccess', () => {
      fetch(targetEmployeeId, targetDate)(dispatcherMock.dispatch);
    ...
    describe('delete paid holiday', () => {
      describe('Whether the deletion was performed correctly when the "Yes" button was pressed.', () => {
        ...
        beforeAll(() => {
          ...
          store.dispatch(
            // @ts-ignore
            deleteGrantedPaidHoliday('xxxxxxx', 'xxxxx', targetDate, confirm)
          );
        });
