      ...
      if (
        ...
        const placeholderLabel = (() => {
          if (config?.placeholderLabel) {
            return (
              msg()[config.placeholderLabel] || msg().Admin_Lbl_PleaseSelect
            );
          }

          return msg().Admin_Lbl_PleaseSelect;
        })();

        // if no options in list and has custom placeholder config
        // first option will be custom placeholder option
        if (!firstOption && config?.placeholderLabel) {
          Array.isArray(options) &&
            options.unshift({ text: placeholderLabel, value: '' });
        }

        // @ts-ignore
        if (firstOption && firstOption.text !== placeholderLabel) {
          // @ts-ignore
          options.unshift({ text: placeholderLabel, value: '' });
        }
      }
