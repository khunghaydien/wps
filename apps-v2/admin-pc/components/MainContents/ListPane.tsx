...
export default class ListPane extends React.PureComponent<Props> {
              ...
              if (item[config.dependent]) {
                ...
                if (_.isArray(item[config.key])) {
                  ...
                  if (
                    ...
                    const option =
                      _.find(options, { value: item[config.key] }) || {};
                    row[config.key] = msg()[option.label];
                  } else if (config.options) {
                    const option =
                      _.find(config.options, {
                        value: item[config.key],
                      }) || {};
                    row[config.key] = msg()[option.msgkey];
                  } else {
                    row[config.key] = item[config.key];
                  }
                }
  ...
  renderTargetDateArea() {
    if (!_.isNil(this.props.configList.history) || this.props.hasTargetDate)
      return (
    ...
    return (
      <div className={`${ROOT}`}>
        <ListPaneHeader
          ...
          historyArea={this.renderTargetDateArea()}
