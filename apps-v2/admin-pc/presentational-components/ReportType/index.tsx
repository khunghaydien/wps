...
export default class ReportType extends React.Component<Props> {
  componentDidMount() {
    const { actions, companyId } = this.props;
    const param = { companyId };

    actions.search(param);
    actions.searchExtendedItem(param);
    actions.searchPaymentMethod({ ...param, active: true });
    actions.getConstantsVendorUsed();
    actions.getConstantsCostCenterUsed();
    actions.getConstantsJobUsed();
    actions.searchPrintPageLayout();
  }
  ...
  UNSAFE_componentWillUpdate(nextProps) {
    if (this.props.companyId !== nextProps.companyId) {
      ...
      this.props.actions.searchPaymentMethod({ ...param, active: true });
    }
    if (
