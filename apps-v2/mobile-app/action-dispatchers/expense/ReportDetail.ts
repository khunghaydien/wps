...
import { actions as reportActions } from '@apps/domain/modules/exp/request/report';
...
export const preProcess =
  (reportId: string, isRequest?: boolean) => (dispatch: AppDispatch) => {
    return dispatch(
      withLoading(reportActions.preProcess(reportId, isRequest))
    ).catch((err) => err);
  };
