...
const FilePreview = (props: Props) => {
  ...
  const renderSVG = (Component, className?: string) => (
    <Component
      className={classNames(`${ROOT}__icon`, className)}
      aria-hidden="true"
    />
  );
    ...
    const attachmentContent = (
      <>
        ...
        {isNotImageType ? (
          ...
          <div className={`${ROOT}__img-preview-container`}>
            <img
              alt={imgAltText}
            ...
            />
            {renderSVG(IconIMG, `${ROOT}__icon-absolute`)}
          </div>
        )}
        {withOCR && fullScreenPreview && renderFullPreviewOCRInfo()}
      </>
    );
    attachmentContainer = noSection ? (
      <div className={className}>{attachmentContent}</div>
    ) : (
      <>
