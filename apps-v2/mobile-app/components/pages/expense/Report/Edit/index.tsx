...
import CashAdvanceArea from '@mobile/components/molecules/expense/CashAdvanceArea';
...
export type Props = {
  ...
  showSwitchReportTypeConfirmDialog: (
    isShowRemovePaymentMethodMsg: boolean
  ) => Promise<boolean>;
  // Update Info
...
export default class ReportEditPage extends React.Component<Props, State> {
  ...
  async onChangeExpReportType(
    ...
    const isUseCashAdvance = get(selectedReportType, 'useCashAdvance', false);
    const cashAdvanceData = !isUseCashAdvance ? initialCashAdvanceData : {};
  ...
  handleSaveClick = async () => {
    // show confirm dialog on expense type switch
    const {
      handleSubmit,
      expReportTypeList,
      showSwitchReportTypeConfirmDialog,
    ...
    const originalReportType = find(expReportTypeList, [
      'id',
      targetExpReport.expReportTypeId,
    ]);
    const currentReportType = find(expReportTypeList, [
      'id',
      values.report.expReportTypeId,
    ]);

    const isNewReport = !values.report.reportId;
    const isChangeReportType = originalReportType.id !== currentReportType.id;
    const hasPaymentMethodInOriginal =
      this.hasPaymentMethod(originalReportType);
    if (!isNewReport && isChangeReportType && hasPaymentMethodInOriginal) {
      const isShowRemovePaymentMethodMsg =
        hasPaymentMethodInOriginal && !this.hasPaymentMethod(currentReportType);
      const isConfirm = await showSwitchReportTypeConfirmDialog(
  ...
  renderTotalAmount = () => {
    const {
      isRequest,
      userSetting: { currencySymbol, currencyDecimalPlaces },
      values: { report },
    } = this.props;
      ...
      <>
        {!isRequest ? (
          <ViewItem label={msg().Exp_Lbl_TotalAmount}>
            <p className={`${ROOT}__total-amount-text`}>
  ...
  render() {
    ...
    const isShowCustomRequest = !isRequest && isCustomRequestVisible;
    ...
    const isShowSettlementAmount =
      (report.isUseCashAdvance || alwaysDisplaySettlementAmount) && !isRequest;
        ...
        <Form className="main-content">
          ...
          {report.isUseCashAdvance && (
            <section className={`${ROOT}__input`}>
              <CashAdvanceArea
                ...
                isRequest={isRequest}
                report={report}
                getCustomHint={this.getCustomHintProps}
                onChangeValue={this.updateReportValues}
          ...
          {isShowSettlementAmount && (
            <section className={`${ROOT}__input`}>
              ...
              >
                <p className={`${ROOT}__amount-text`}>{settlementAmount}</p>
              </ViewItem>
            </section>
          )}
