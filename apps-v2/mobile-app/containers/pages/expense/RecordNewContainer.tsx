...
import recordSchema from '@mobile/schema/RecordNewSchema';
...
import { actions as selectedIcCardActions } from '@mobile/modules/expense/ui/icCard/selectedCard';
...
const RecordNewContainer = (ownProps: ContainerProps) => {
  useEffect(() => {
    ...
    if (ownProps.isUnderApprovedPreRequest) {
      ownProps.isReadOnly(true);
    }
    const isMileage = isMileageRecord(ownProps.values.recordType);

    // get payment method option list for record detail page
    ...
    if (isExpTypeChanged) {
      ...
      if (
        ...
        Promise.all([
          ...
          ownProps.getExpenseTypeById(expTypeId, 'REPORT'),
  ...
  return (
    <RecordNewPage
      ...
      openCCTransactionDialog={ownProps.openCCTransactionDialog}
      openICTransactionDialog={ownProps.openICTransactionDialog}
...
const mapStateToProps = (state: State, ownProps: OwnProps) => {
  ...
  const selectedReportType = state.expense.entities.selectedReportType;
  ...
  const navigateToRecordPage = () => {
    ...
    const path = !formValues.recordId
      ? '/expense/report/record/new/general'
      : `/expense/record/detail/${reportId}/${recordId}`;
  ...
  return {
    ...
    icCardList: state.expense.entities.icCard,
    selectedCCTrans: state.expense.ui.creditCard.selectedTransactions[0],
...
const mapDispatchToProps = {
  ...
  getICCardList,
  getICTransactions,
  setSelectedIcCard: selectedIcCardActions.set,
  getPaymentMethodOptionList,
  removeInactiveFromOptionList,
  setSelectedCardName: cardNameActions.set,
  clearSelectedCardName: cardNameActions.clear,
  clearSelectedDetail: detailActions.clear,
  clearSelectedStatus: statusActions.clear,
  clearSelectedRequestDate: requestDateActions.clear,
  endLoading,
  startLoading,
};

const setValuesForFC = (
...
const mergeProps = (
  ...
  onClickSearchExpType: () => {
    ...
    const path = `/expense/expense-type/list/${RECORD_TYPE_CATEGORY.noJorudan}`;
    pushHistoryWithPrePage(history, path, {
      target,
    });
  },
  openReceiptLibrary: () => {
    const path = `/expense/receipt-library/list/backType=${backType.RECORD}`;
    pushHistoryWithPrePage(ownProps.history, path);
  },
  openMileageMap: (values: Record) => {
    ...
    history.replace({
      ...
      state: {
        ...
        target,
        isApprovedPreRequest: stateProps.isUnderApprovedPreRequest,
      },
    });
  ...
  navigateToItemizationPage: (idx: number, values: Record) => {
    ...
    dispatchProps.saveFormValues(values);
    const path = `/expense/record/item/${idx}`;
    const state = {
  ...
  onDeleteClick: () => {
    const { isUseCashAdvance } = stateProps;
    dispatchProps
      .deleteRecord(ownProps.recordId, false, isUseCashAdvance)
      // @ts-ignore
      .then(() => {
  ...
  onBackClick: () => {
    if (ownProps.type === UI_TYPE.REPORT_LIST) {
      ...
      ownProps.history.push(
        `/expense/report/detail/${stateProps.formValues.reportId}`
      );
    }
  ...
  onClickSearchCustomEI: (
    ...
    pushHistoryWithPrePage(
      ownProps.history,
      `/expense/customExtendedItem/list/backType=${backType}/reportId=${reportId}/recordId=${recordId}/itemIdx=${itemIdx}/index=${index}/customExtendedItemLookupId=${customExtendedLookupId}/customExtendedItemId=${customExtendedItemId}/customExtendedItemName=${customExtendedItemName}`,
      {
        type: ownProps.type,
        target: get(ownProps, 'location.state.target'),
      }
  ...
  onClickSearchCostCenter: (recordDate: string) => {
    ...
    const path = `/expense/cost-center/list/backType=record/targetDate=${recordDate}/reportId=${reportId}`;
    pushHistoryWithPrePage(ownProps.history, path, {
      type: ownProps.type,
      target: get(ownProps, 'location.state.target'),
      itemIdx: itemIdx || 0,
  ...
  onClickCloneRecord: (numberOfDays?: number, targetDates?: string[]) => {
    ...
    dispatchProps
      ...
      // @ts-ignore
      .then((_res) => {
        history.replace(`/expense/report/detail/${reportId}`);
      })
      .catch((err) => {
        const errMsg =
          (err.message && ` (${err.message})`) ||
  ...
  openCCTransactionDialog: (values: Record) => {
    dispatchProps.saveFormValues(values);
    const { report } = stateProps;
    const reportId = ownProps.reportId || report.reportId || undefined;
    dispatchProps.clearSelectedCardName();
    dispatchProps.clearSelectedDetail();
    dispatchProps.clearSelectedStatus();
    dispatchProps.clearSelectedRequestDate();
    pushHistoryWithPrePage(
      ownProps.history,
      `/expense/credit-card/transactions/${reportId}/${ownProps.recordId}`,
      {
        type: 'add',
      }
    );
  },
  openICTransactionDialog: (values: Record) => {
    dispatchProps.saveFormValues(values);
    const { salesId, customerId, companyId, employeeCode, report } = stateProps;
    const reportId = ownProps.reportId || report.reportId || undefined;
    ...
    dispatchProps
      .getICCardList(salesId, customerId, companyId, employeeCode)
      // @ts-ignore
      .then((res) => {
        const cards = (res && res[0]) || [];
        dispatchProps.setSelectedIcCard(cards.map(({ cardNo }) => cardNo));
        dispatchProps.setSelectedCardName(cards.map(({ cardNo }) => cardNo));
        pushHistoryWithPrePage(
          ownProps.history,
          `/expense/ic-card/transactions/${reportId}/${ownProps.recordId}`,
          {
            type: 'add',
          }
        );
  ...
  onClickSearchVendor: (backType: string) => {
    ...
    pushHistoryWithPrePage(
      ownProps.history,
      `/expense/vendor/search/reportId=${reportId}/backType=${backType}`
    );
  },
  onClickVendorDetail: (values: Record) => {
    const { history } = ownProps;
    const vendorId = get(values, 'items.0.vendorId');
    pushHistoryWithPrePage(history, `/expense/vendor/detail/${vendorId}`);
  },
});
...
export default compose<any>(
  ...
  withFormik({
    ...
    handleSubmit: async (values, { props }) => {
      ...
      saveValues.useCashAdvance = props.isUseCashAdvance;
      ...
      if (isAddedRecord) {
        props
          .createRecord(saveValues, reportId, expReportTypeId, props.employeeId)
          // @ts-ignore
          .then(() => {
            props.clearFormValues();
            props.resetCustomHint();
            if (isAddedRecord) {
              props.history.push(`/expense/report/detail/${values.reportId}`);
