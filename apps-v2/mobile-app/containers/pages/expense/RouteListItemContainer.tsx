...
const RouteListItemContainer = (ownProps: OwnProps & FormikValues) => {
  ...
  useEffect(() => {
    ...
    dispatch(getPaymentMethodOptionList(ownProps.values, true));
  }, []);
...
const mergeProps = (
  ...
  onClickBackButton: (values: Record) => {
    ...
    if (ownProps.type === UI_TYPE.ADD) {
      ...
      pushHistoryWithPrePage(history, '/expense/report/route/list', stateParam);
    } else if (ownProps.type === UI_TYPE.REPORT) {
      history.push(`/expense/report/detail/${ownProps.reportId}`);
    }
  },
  onClickEditButton: () => {
  ...
  onClickChangeRouteBtn: (
    ...
    ownProps.history.push(
      `/expense/report/route/edit/${ownProps.recordId}/${ownProps.reportId}`,
      stateParam
    );
  ...
  getValuesWithSelectedRoute: (routeNo: string) => {
    ...
    record.paymentMethodId = routeFormParams.paymentMethodId;
  ...
  onClickSearchCustomEI: (
    ...
    pushHistoryWithPrePage(
      ownProps.history,
      `/expense/customExtendedItem/list/backType=${backType}/reportId=${reportId}/recordId=${recordId}/itemIdx=null/index=${index}/customExtendedItemLookupId=${customExtendedItemLookupId}/customExtendedItemId=${customExtendedItemId}/customExtendedItemName=${customExtendedItemName}`,
      {
        type: ownProps.type,
        target: _.get(ownProps, 'location.state.target'),
      }
  ...
  onClickSearchCostCenter: (recordDate: string) => {
    ...
    const path = `/expense/cost-center/list/backType=${backType}/targetDate=${recordDate}/reportId=${reportId}`;
  ...
  onClickSearchJob: (recordDate: string) => {
    ...
    pushHistoryWithPrePage(
      ownProps.history,
      `/expense/job/list/backType=${backType}/targetDate=${recordDate}/reportId=${reportId}`,
      {
        type: ownProps.type,
        target: _.get(ownProps, 'location.state.target'),
      }
  ...
  removeInactivePaymentMethod: (selectedPaymentMethodId: string) => {
    ...
    const reportTypePaymentMethodIds = _.get(
      selectedReportType,
      'paymentMethodIds'
    );
...
export default compose(
  ...
  withFormik({
    ...
    handleSubmit: (values, { props, setStatus }) => {
      ...
      props
        .createRecord(result, null, null, props.employeeId)
        // @ts-ignore
        .then(() => {
