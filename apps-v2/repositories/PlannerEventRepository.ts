...
const convertFromMicrosoft365 = (event: Event): Event => {
  if (
    (event.createdServiceBy === 'microsoft365' ||
      event.createdServiceBy === 'salesforce') &&
    event.isAllDay
  ) {
    const asUTC = (date) => {
      return parse(
...
export default {
  ...
  */
  ...
  fetch: async (param: {
    ...
    return {
      events: toViewModel(events.map(convertFromMicrosoft365)),
