...
export const searchRoute =
  ...
  (dispatch: AppDispatch) => {
    ...
    if (!param.origin) {
      if (tmpOrigin) {
        searchStationList.push(
          searchStation(tmpOrigin, param.targetDate, null, empHistoryId)
            .then((result: Station) => {
              if (result.num > 0) {
    ...
    if (!param.arrival) {
      if (tmpArrival) {
        searchStationList.push(
          searchStation(tmpArrival, param.targetDate, null, empHistoryId)
            .then((result: Station) => {
              if (result.num > 0) {
                const station = result.stationList[0];
                (param.arrival as any) = station;
    ...
    tmpViaList.forEach((tmpVia, idx) => {
      if (tmpVia && !param.viaList[idx]) {
        searchStationList.push(
          searchStation(tmpVia, param.targetDate, null, empHistoryId)
            .then((result: Station) => {
              if (result.num > 0) {
                const station = result.stationList[0];
    ...
    if ((param.origin || tmpOrigin) && (param.arrival || tmpArrival)) {
      ...
      return Promise.all(searchStationList).then((result) => {
        if (
          param.origin &&
          param.arrival &&
          ...
          return dispatch(routeActions.search(param))
            .then((response) => {
              if (openRouteDialog) dispatch(activeDialogActions.routeSelect());
              return response;
            })
            .catch((err) =>
              dispatch(catchApiError(err, { isContinuable: true }))
...
export const onChangeTmpOrigin =
  (value: string, isClear: boolean) => (dispatch) => {
