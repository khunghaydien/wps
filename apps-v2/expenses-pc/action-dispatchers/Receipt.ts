...
export const executeOcr =
  (receiptFileId: string, empHistoryId?: string) => (dispatch: AppDispatch) => {
    dispatch(loadingStart());
    return dispatch(
      receiptLibraryAction.executeOcr(receiptFileId, empHistoryId)
    )
      .then((res) => res)
      .catch((err) => dispatch(catchApiError(err, { isContinuable: true })))
...
export const keepGettingPdfStatus =
  ...
  (dispatch: AppDispatch, getState: () => State) => {
    return dispatch(
      ...
      // update scanned pages when user navigates between receipt & receipt detail dialogs
