...
beforeEach(() => {
  ...
  const path = {
    ...
    accountingPeriod: '/exp/accounting-period/search',
  };
  const api = {
    param1: {
  ...
  ApiMock.setDummyResponse(path.accountingPeriod, api.param4, response);
});
...
describe('report summary cost center', () => {
  ...
  it('cost center search dialog open', async () => {
    ...
    await act(async () => firstItem.simulate('click'));
    await act(async () => expenseForm.update());
    expect(
      expenseForm.find(costCenterInputFieldSelector).at(0).props().value
    ).toEqual(
...
describe('file attachment', () => {
  ...
  const addFileSelector = 'ts-expenses__form-report-summary__actions__add_file';
  ...
  it('clear selected attachment', () => {
    ...
    expenseForm
      .find(clearAttachmentSelector)
      .children()
      .props()
      .onMouseDown({
        preventDefault: () => {},
        currentTarget: {
          value: mockAttachmentReport.attachedFileList[0].attachedFileId,
        },
      });
    expenseForm.update();
...
describe('actions', () => {
  ...
  it('close report', async () => {
    ...
    expect(expense.exists(reportSummarySelector)).toBeTruthy();
    expect(expense.exists(closeButtonSelector)).toBeTruthy();
    ApiMock.setDummyResponse(api.path, api.param, { records: [] });

    expense.find(closeButtonSelector).props().onClick();
    expense.update();
    expect(expense.exists(reportListSelector)).toBeTruthy();
  ...
  it('clone', async () => {
    ...
    expenseForm.find(cloneButtonSeletor).simulate('click');
    await act(async () => expenseForm.update());
    expect(ApiMock.invoke).toHaveBeenCalledWith(api);
  ...
  it('submit call attendance check first and display approval dialog', async () => {
    ...
    ApiMock.setDummyResponse('/exp/report/approval/pre-process', {
      reportId: 'a0w0o00000UAQqwAAH',
    });
    const expenseForm = renderComponent(store);
  ...
  it('request submit', async () => {
    ...
    requestButton.at(0).simulate('click');
  ...
  it('recall', async () => {
    ...
    recallButton.at(0).simulate('click');
  ...
  it('delete', async () => {
    ...
    await act(async () => expense.find(confirmButton).simulate('click'));

    await act(async () => expense.update());
    expect(ApiMock.invoke).toHaveBeenCalledWith({
