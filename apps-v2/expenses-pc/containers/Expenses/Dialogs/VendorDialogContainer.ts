...
const mapStateToProps = (state: State, ownProps: OwnProps) => ({
  ...
  backType: state.ui.expenses.dialog.vendor.backType,
...
const mergeProps = (
  ...
  onVendorSearch: (query) =>
    // @ts-ignore
    dispatchProps.searchVendorLookup(stateProps.selectedCompanyId, query),
  onClickVendorItem: (item) => {
    ...
    if (stateProps.backType === vendorBackTypes.RECORD_VENDOR) {
      const expRecord = cloneDeep(
        ownProps.expReport.records[ownProps.recordIdx]
      );
      const targetRecord = `report.records[${ownProps.recordIdx}]`;
      expRecord.items[0].vendorId = id;
      expRecord.items[0].vendorName = name;
      expRecord.items[0].vendorCode = code;
      expRecord.items[0].paymentDueDateUsage = paymentDueDateUsage;
      expRecord.items[0].paymentDueDate = '';
      if (stateProps.useJctRegistrationNumber) {
        if (jctRegistrationNumber) {
          expRecord.items[0].jctRegistrationNumber = jctRegistrationNumber;
        }
        expRecord.items[0].vendorIsJctQualifiedIssuer =
          isJctQualifiedInvoiceIssuer;
        expRecord.items[0].vendorJctRegistrationNumber = jctRegistrationNumber;
      }

      ownProps.onChangeEditingExpReport(targetRecord, expRecord);
    } else if (stateProps.backType === vendorBackTypes.RECORD_MERCHANT) {
      const expRecord = cloneDeep(
        ownProps.expReport.records[ownProps.recordIdx]
      );
      const targetRecord = `report.records[${ownProps.recordIdx}]`;
      expRecord.items[0].merchant = name;
      ownProps.onChangeEditingExpReport(targetRecord, expRecord);
    } else {
      ...
      if (stateProps.useJctRegistrationNumber) {
        expReport.vendorJctRegistrationNumber = jctRegistrationNumber;
        expReport.vendorIsJctQualifiedIssuer = isJctQualifiedInvoiceIssuer;
      }
    ...
    dispatchProps.getPersonalVendor(id).then((item) => {
      ...
      if (stateProps.backType === vendorBackTypes.RECORD_VENDOR) {
        ...
        if (stateProps.useJctRegistrationNumber) {
          ...
          expRecord.items[0].vendorIsJctQualifiedIssuer =
            isJctQualifiedInvoiceIssuer;
          expRecord.items[0].vendorJctRegistrationNumber =
            jctRegistrationNumber;
        }
        ownProps.onChangeEditingExpReport(targetRecord, expRecord);
      } else if (stateProps.backType === vendorBackTypes.RECORD_MERCHANT) {
  ...
  onClickDeleteVendorItem: (vendorInfo: Vendor) => {
    const { subroleId } = stateProps;
    const item = stateProps.personalList.find((o) => o.id === vendorInfo.id);
    dispatchProps.deleteVendor(item, subroleId);
  },
