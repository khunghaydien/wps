...
export const submitMemo =
  (roleId: string, memoId: string, memo: string, memoType: string) =>
  async (dispatch: AppDispatch) => {
    dispatch(loadingStart());
    try {
      await dispatch(roleActions.submitMemo(memoId, memo, memoType));
...
export const rescheduleRole =
  (roleRescheduleParam: RoleRescheduleParam) => (dispatch: AppDispatch) => {
    ...
    dispatch(roleActions.reschedule(roleRescheduleParam))
      .then(() => {
        dispatch(siteRouteActions.showRoleDetails());
        dispatch(roleActions.get(roleRescheduleParam.roleId));
      })
      .catch((err) => dispatch(catchApiError(err, { isContinuable: true })))
      .finally(() => dispatch(loadingEnd()));
  };
...
export const saveRole =
  (role: Role, projectId: string, jobGradeList: Array<any>, mode?: string) =>
  async (dispatch: AppDispatch) => {
    dispatch(loadingStart());
    try {
      ...
      await Promise.all([
        dispatch(activeDialogActions.hide()),
        mode === 'ROLE_DETAILS_FROM_FINANCE'
