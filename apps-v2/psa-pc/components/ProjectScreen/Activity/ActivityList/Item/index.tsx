...
function ActivityItem(props: Props) {
  ...
  const riskStatusClass = classNames(`${ROOT}__progress-status`, {
    'is-critical': props.riskStatus === ActivityRiskStatus.Critical,
    'is-high': props.riskStatus === ActivityRiskStatus.High,
    'is-medium': props.riskStatus === ActivityRiskStatus.Medium,
    'is-low': props.riskStatus === ActivityRiskStatus.Low,
  });
    ...
    >
      ...
      >
        <div className={`${ROOT}__name`}>
          ...
          <div className={`${ROOT}__name-text`}>
            {props.title}
            <div>
              {props.enableProgressCheck &&
                props.riskStatus &&
                props.status &&
                props.status !== ActivityStatus.NotStarted &&
                (props.progressComment ? (
                  <Tooltip
                    align={'bottom left'}
                    content={props.progressComment}
                  >
                    <div className={riskStatusClass}>
                      {msg()[`Psa_Lbl_RiskStatus${props.riskStatus}`]}
                    </div>
                  </Tooltip>
                ) : (
                  <div className={riskStatusClass}>
        ...
        <div className={`${ROOT}__status`}>
          {props.status &&
          props.status === ActivityStatus.InProgress &&
          props.enableProgressCheck &&
          typeof props.progressLevel === 'number' ? (
            <>
              <progress
                className={`${ROOT}__activity-progress`}
                value={props.progressLevel}
                max={100}
              />
              <div className={`${ROOT}__status-label`}>
                {props.progressLevel}%
              </div>
            </>
          ) : (
