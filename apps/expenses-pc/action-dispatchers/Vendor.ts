...
export const getRecentVendors =
  (empId: string, isPersonalVendor?: boolean) => (dispatch: AppDispatch) => {
    return getRecentlyUsedVendor(empId, isPersonalVendor)
      .then((result) => {
...
export const searchVendorLookup =
  (companyId: string, query: string) => (dispatch: AppDispatch) => {
    ...
    return getVendorList(companyId, query)
      .then((result: VendorItemList) => {
        return result;
      })
      .catch((err) => dispatch(catchApiError(err, { isContinuable: true })))
...
export const saveVendor =
  (vendor: Vendor, empId: string) =>
  (dispatch: AppDispatch): void => {
    dispatch(personalVendorActions.save(vendor, empId))
      .then(({ id, name }) => {
...
export const updateVendor =
  (vendor: Vendor, empId: string) =>
  (dispatch: AppDispatch): void => {
    dispatch(personalVendorActions.update(vendor, empId))
      .then(({ name }) => {
        const vendorCopy = { ...vendor };
        vendorCopy.name = name;
...
export const deleteVendor =
  ...
  (dispatch: AppDispatch): void => {
    dispatch(loadingStart());
    dispatch(personalVendorActions.delete(vendor.id))
      .then(() => {
...
export const undoVendorDeletion =
  ...
  (dispatch: AppDispatch): void => {
    ...
    dispatch(loadingStart());
    dispatch(personalVendorActions.restore(vendor.id))
      .then(() => {
