...
export const backToProjectList =
  ...
  (dispatch: AppDispatch) => {
    ...
    dispatch(projectActions.list(companyId, pageNo, psaGroupId, filterQuery))
      .then(() => {
...
export const fetchProjectList =
  ...
  (dispatch: AppDispatch) => {
    dispatch(loadingStart());
    return dispatch(
      projectActions.list(companyId, pageNo, psaGroupId, filterQuery, pageSize)
    )
      .then((res) => {
...
export const saveProject =
  (project: Project, companyId: string, psaGroupId: string) =>
  async (dispatch: AppDispatch) => {
    dispatch(loadingStart());
    try {
      const res = await dispatch(projectActions.save(project));
      await Promise.all([
        dispatch(projectActions.list(companyId, 1, psaGroupId)),
