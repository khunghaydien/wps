...
const convertSummaries = (summaries) => {
  const $summaries = summaries
    // 出生時育児休業（産後パパ育休）は警告用の項目なので表示させない
    .filter(
      (summary) =>
...
const convertAttentions = (records, summaries) => {
  ...
  summary.ineffectiveWorkingTime = allIds.filter((id) =>
    byId[id].some(
      ({ code }) => code === ATT_DAILY_ATTENTION_CODE.IneffectiveWorkingTime
    )
  ).length;
  summary.insufficientRestTime = allIds.filter((id) =>
    byId[id].some(
      ({ code }) => code === ATT_DAILY_ATTENTION_CODE.InsufficientRestTime
    )
  ).length;

  const paternityLeaveAtBirthSummary = summaries.find(
    (summary) => summary.name === 'PaternityLeaveAtBirthSummary'
  );
  if (paternityLeaveAtBirthSummary) {
    summary.paternityLeaveAtBirthSummary =
      paternityLeaveAtBirthSummary.items.map((item) => ({
        id: item.name,
        startDateWorkTime: item.items.find(
          ({ name }) => name === 'startDateWorkTime'
        )?.value,
        endDateWorkTime: item.items.find(
          ({ name }) => name === 'endDateWorkTime'
        )?.value,
        totalWorkTime: item.items.find(({ name }) => name === 'totalWorkTime')
          ?.value,
        totalWorkDays: item.items.find(({ name }) => name === 'totalWorkDays')
          ?.value,
      }));
  }

  const childCareAllowanceSummary = summaries.find(
    (summary) => summary.name === 'ChildCareAllowanceSummary'
  );
  if (childCareAllowanceSummary) {
    summary.childCareAllowanceSummary = childCareAllowanceSummary.items.map(
      (item) => ({
        id: item.name,
        totalWorkTime: item.items.find(({ name }) => name === 'totalWorkTime')
          ?.value,
  ...
  const childCareSummary = summaries.find(
    (summary) => summary.name === 'ChildCareSummary'
  );
  if (childCareSummary) {
    summary.childCareSummary = childCareSummary.items.map((item) => ({
      id: item.name,
      totalWorkDays: item.items.find(({ name }) => name === 'totalWorkDays')
        ?.value,
    }));
...
const convertDetailEntity = (body) => {
  const detailEntity = {
    ...
    attentions: convertAttentions(body.records, body.summaries),
  };
