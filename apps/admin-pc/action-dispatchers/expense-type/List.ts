...
import { actions as IdsActions } from '../../modules/expense-type/entities/ids';
import { actions as ListActions } from '../../modules/expense-type/entities/list';
import { actions as TotalCountActions } from '../../modules/expense-type/entities/totalCount';
import {
  actions as SearchConditionActions,
  initialState as searchConditionInitialState,
  State as SearchCondition,
} from '../../modules/expense-type/ui/searchCondition';
import {
  actions as SearchQueryActions,
  State as SearchQuery,
} from '../../modules/expense-type/ui/searchQuery';
...
/**
 * This method is used to search ExpenseType IDs
 * with the same SearchQuery as that when used search() method
 * and fetch ExpenseType Records.
 *
 * @param {*} searchQuery
 * @param {*} pagingOptions
 */
...
export const showBaseCurrencyValidationErr = () => (dispatch: AppDispatch) => {
  dispatch(
    catchBusinessError(
      msg().Admin_Lbl_ValidationCheck,
      msg().Exp_Msg_NoBaseCurrencyForExpense,
      null
    )
  );
};
