...
const ViewAllResourcesContainer = () => {
  ...
  const requireOwnerIdForGetGroupList = useSelector((state: State) => {
    if (state.ui.tab === 'Projects') {
      return false;
    } else if (
      state.ui.tab === 'Resource' &&
      state.entities.psa.setting.allowCrossGroupSearch
    ) {
      return false;
    } else {
      return true;
  ...
  useEffect(() => {
    dispatch(
      initializePagination(companyId, employeeId, CALL_BY.RM, selectedGroup.id)
    );
  }, []);
  ...
  return (
    <ViewAllResources
      ...
      updateResourceSelectionState={updateResourceSelectionState}
      viewAllResourcesFilterState={viewAllResourcesFilterState}
      isFetching={isFetching}
      selectedGroup={selectedGroup}
      requireOwnerIdForGetGroupList={requireOwnerIdForGetGroupList}
