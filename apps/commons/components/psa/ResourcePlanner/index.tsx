...
type Props = {
  ...
  applyFilter: (
    nextState: ResourceSelectionFilterState,
    loadmore?: boolean
  ) => void;
...
const ResourcePlanner = (props: Props) => {
  ...
  const reachFetchLimit =
    props.resourceList &&
    props.resourceList.length > 0 &&
    props.resourceList.length % 50 !== 0;
  const reachMaxLimit = props.resourceList && props.resourceList.length >= 300;
  ...
  const hasScheduleResult =
    props.roleScheduleResult &&
    props.roleScheduleResult.availableTime &&
    props.roleScheduleResult.availableTime.length > 0;
    ...
    <div className={`${ROOT}`}>
      ...
      <div className="ts-psa-fixed-header">
        <PSACommonHeader title={viewDuration}>
          ...
          <ViewSelector
            endDate={
              hasScheduleResult ? scheduledEndDate : reduxFilterState.endDate
            }
        ...
        <ResourcePlannerFilterInfo
          ...
          minDate={props.selectedRole.startDate}
        ...
        {!props.isLoading && (
          <div className={`${ROOT}__resource-container`}>
            <div className={`${ROOT}__resource-list-content`}>
              ...
              {showSearchResource && hasMatchingResources && (
                <div className={`${ROOT}__load-more-container`}>
                  {!props.loadingMore ? (
                    <Button
                      disabled={reachFetchLimit || reachMaxLimit}
                      className={`${ROOT}__load-more-btn`}
                      data-testid={`${ROOT}__load-more-btn`}
                      onClick={() => {
                        props.applyFilter(
                          props.resourceSelectionFilterState,
                          true
                    ...
                    >
                      {msg().Psa_Btn_Loadmore}
                    </Button>
                  ) : (
                    <div className={`${ROOT}__load-more-spinner`}>
                      <div
                        role="status"
              ...
              <div className={`${ROOT}__limit-reach`}>
                {props.resourceList &&
                  props.resourceList.length >= 300 &&
                  msg().Psa_Msg_LimitReach}
              </div>
            </div>
          </div>
        )}
