...
const RoleDetailsBtnArea = (props: Props) => {
  ...
  const allowForReverseAssign =
    props.currentMode &&
    props.currentMode === modes.RESOURCE_ASSIGNMENT &&
    props.selectedResourceCount === 1;
  const toggleMenu = () => {
    setMenu(!isMenuOpen);
  };
      ...
      {isMenuOpen && isResourceRoute && (
        <div className={`${ROOT}__submenu`}>
          ...
          <button
            ...
            disabled={
              isBtnDisabled.softBooking ||
              (!allowForReverseAssign &&
                (props.selectedResourceCount !== 1 ||
                  isBtnDisabled.selectResource))
            }
          >
