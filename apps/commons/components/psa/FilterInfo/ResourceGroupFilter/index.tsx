...
const ResourceGroupFilter = (props: Props) => {
  ...
  const psaGroupId = useSelector(
    (state: resourceState) => state.entities.psa.psaGroup.selectedGroup.id
  );
  ...
  useEffect(() => {
    if (isDialogOpen) {
      switch (siteRoute) {
        case 'REQUEST_LIST':
          dispatch(getResourcegroups(companyId, psaGroupId, employeeId));
          break;

        case 'VIEW_ALL_RESOURCES':
          tab === 'Projects' || (allowCrossGroupSearch && tab === 'Resource')
            ? dispatch(getResourcegroups(companyId, psaGroupId))
            : dispatch(getResourcegroups(companyId, psaGroupId, employeeId));
          break;
        ...
        case 'RESOURCE_PLANNER':
          dispatch(getResourcegroups(companyId, psaGroupId, selectedRole.rmId));
          break;
      }
    }
  }, [isDialogOpen]);
  ...
  const initSelectedGroup = () => {
    if (props.resourceGroups) {
      ...
      if (length >= 0 && props.groupList.length > 0) {
        setSelection(
          new Set(
        ...
        setSelection(new Set(props.resourceGroups));
      }
  ...
  useEffect(() => {
    ...
    initSelectedGroup();
  }, [props.groupList, props.disabled]);
