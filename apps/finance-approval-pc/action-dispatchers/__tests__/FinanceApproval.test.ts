import { addInitialValue } from '../../modules/entities/advSearchConditionList';
...
describe('FinanceApproval', () => {
    ...
    it('saveSearchCondition success', async () => {
      ...
      await store.dispatch(
        actions.saveSearchCondition(
          addInitialValue(searchConditionRes.searchConditionList as any)
        )
      );
      expect(store.getActions()).toMatchSnapshot();
    });
  ...
  describe('saveExpReport()', () => {
    it('saveExpReport success', async () => {
      ...
      await store.dispatch(actions.saveExpReport(reportDetail, '', 'Asc', ''));
      expect(store.getActions()).toMatchSnapshot();
    });
  });
