...
function getStartTimeInMinutes(event: EventViewModel): number {
  // 予定開始日の 23:30 の Date
  const thirtyMinutesAgo = subMinutes(endOfDay(event.startDateTimeOfDay), 30);
  const isAfter2330 = isAfter(event.startDateTimeOfDay, thirtyMinutesAgo);
  if (isAfter2330) {
    // 23:30 以降の予定であれば、UI上の開始位置を23:30の位置にする
    return thirtyMinutesAgo.getHours() * 60 + thirtyMinutesAgo.getMinutes();
  }
  return (
