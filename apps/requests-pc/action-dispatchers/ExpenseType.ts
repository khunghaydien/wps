...
export const favoriteExpType =
  (empId: string, expType: ExpenseType) => (dispatch: AppDispatch) => {
    dispatch(expenseTypeSelectListActions.favorite(expType));
    return favoriteExpenseType(empId, expType.id, 'ExpenseType')
      .then(() => {
        dispatch(
...
export const unfavoriteExpType =
  (empId: string, expType: ExpenseType) => (dispatch: AppDispatch) => {
    dispatch(expenseTypeSelectListActions.unfavorite(expType.id));
    return unfavoriteExpenseType(empId, expType.id, 'ExpenseType').catch(
      (err) => {
        dispatch(expenseTypeSelectListActions.favorite(expType));
        dispatch(catchApiError(err, { isContinuable: true }));
        throw err;
...
export const getExpenseTypeList =
  (recordType = '', targetDate: string, reportTypeId?: string) =>
  (dispatch: AppDispatch) => {
    dispatch(dialogLoadingActions.toggle(true));
    return dispatch(
...
export const getExpenseTypeById =
  (expTypeId: string) => (dispatch: AppDispatch) => {
    return dispatch(expenseTypeActions.getExpenseTypeById(expTypeId, 'REQUEST'))
      .then((result) => {
        return result;
      })
      .catch((err) => {
