...
import {
  saveVendor,
  updateVendor,
} from '@apps/requests-pc/action-dispatchers/Vendor';
...
const mergeProps = (
  ...
  save: (values) => {
    ...
    dispatchProps.saveVendor(vendor, stateProps.employeeId);
  },
  update: (values) => {
    const vendor = { ...values };
    vendor.paymentDueDateUsage =
    ...
    vendor.companyId = stateProps.companyId;
    vendor.id = vendor.id || null;
    dispatchProps.updateVendor(vendor, stateProps.employeeId);
  },
});
