...
const RouteFormContainer = (ownProps: OwnProps) => {
  ...
  const onClickPushHistory = () => {
    ...
    if (ownProps.type === UI_TYPE.ADD) {
      pushHistoryWithPrePage(history, '/request/report/route/list', { target });
    } else if (ownProps.type === UI_TYPE.REPORT) {
      pushHistoryWithPrePage(
        history,
        `/request/report/route/list/edit/${ownProps.recordId}/${ownProps.reportId}`,
        { target }
      );
    } else {
      pushHistoryWithPrePage(history, '/request/route/list', { target });
    }
  };
  ...
  const onClickBack = () => {
    ...
    if (ownProps.type === UI_TYPE.REPORT) {
      ...
      ownProps.history.push(`/request/report/detail/${formValues.reportId}`);
    }
  };
  ...
  const onClickSearchExpType = () => {
    let target = get(ownProps.history, 'location.state.target');
    target = ownProps.reportId ? 'report' : target;
    const path = `/request/expense-type/list/${RECORD_TYPE.TransitJorudanJP}`;
  ...
  const handleSubmit = (values, { setValues, setStatus }) => {
    ...
    dispatch(searchStations(values)).then((result) => {
      ...
      if (isEmpty(result.errors)) {
        ...
        dispatch(getExpenseTypeById(expenseTypeId, 'REQUEST'));
