...
const RouteListContainer = (ownProps: OwnProps) => {
  ...
  const onClickBackButton = () => {
    ...
    if (ownProps.type === UI_TYPE.ADD) {
      ownProps.history.push(`/request/report/route/new`, { target });
    } else if (ownProps.type === UI_TYPE.REPORT) {
      ownProps.history.push(
        `/request/report/route/edit/${recordId}/${reportId}`,
        { target }
      );
    } else {
      ownProps.history.push(`/request/route/new`, { target });
    }
  };

  const onClickListItem = (routeNo: number) => {
    const { recordId, reportId, type, history } = ownProps;
    let target = get(ownProps.history, 'location.state.target');
    target = ownProps.reportId ? 'report' : target;
    ...
    if (type === UI_TYPE.ADD) {
      pushHistoryWithPrePage(
        history,
        `/request/report/route/list/item/${routeNo}`,
        {
          target,
        }
      );
