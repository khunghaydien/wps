...
const ReportListContainer = (ownProps: OwnProps) => {
  ...
  useEffect(() => {
    if (reportListType === LIST_TYPE.Approved) {
      ...
      boundGetReportList(null, false, REPORT_PER_PAGE_MOBILE, true)
        .then(() => {
          dispatch(getReportIdList(false, employeeId, undefined, true));
        })
        .catch((_err) => false);
    }
  ...
  const onClickApprovedList = () => {
    ...
    dispatch(getReportIdList(true, employeeId, false, true))
      // @ts-ignore
      .then(([ids]) => {
        if (!isEmpty(ids)) {
  ...
  const onRefresh = () => {
    ...
    return (
      dispatch(getReportIdList(isApprovedList, employeeId, true, true))
        // @ts-ignore
