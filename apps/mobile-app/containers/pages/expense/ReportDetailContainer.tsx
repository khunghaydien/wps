...
import { getMobileInitDates as getMobileInitDatesCreditCard } from '@apps/domain/models/exp/CreditCard';
...
import { getCCTransactions } from '../../../action-dispatchers/expense/CreditCard';
import {
  getICCardList,
  getICTransactions,
...
const ReportDetailContainer = (ownProps: OwnProps) => {
  ...
  const onClickCreateICRecord = () => {
    ...
    dispatch(getICCardList(salesId, customerId, companyId, employeeCode))
      // @ts-ignore
      .then((res: Array<IcCards>) => {
        const cards = (res && res[0]) || [];
        if (cards.length === 1) {
          // if only one card, skip card selection
          dispatch(selectedIcCardActions.set(cards[0].cardNo));
          ...
          dispatch(
            ...
            .then(() => {
              pushHistoryWithPrePage(
                ownProps.history,
                `/expense/ic-card/transactions/${ownProps.reportId}`
              );
            });
        } else {
          pushHistoryWithPrePage(
            ownProps.history,
            `/expense/ic-card/list/${ownProps.reportId}`
          );
        }
      });
  };
  ...
  const onClickCreateCCRecord = () => {
    const dates = getMobileInitDatesCreditCard();
    dispatch(
      getCCTransactions(companyId, employeeId, dates.startDate, dates.endDate)
    )
      // @ts-ignore
      .then(() => {
