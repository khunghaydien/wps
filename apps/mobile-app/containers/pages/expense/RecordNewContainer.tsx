...
const mergeProps = (
  ...
  onDeleteClick: () => {
    dispatchProps
      .deleteRecord(ownProps.recordId)
      // @ts-ignore
      .then(() => {
  ...
  onClickLinkBtn: (values: Record) => {
    dispatchProps.saveFormValues(values);
    const { salesId, customerId, companyId, employeeCode } = stateProps;
    dispatchProps
      ...
      .then((res: IcCards) => {
        ...
        if (cards.length === 1) {
          // if only one card, skip card selection
          dispatchProps.setSelectedCard(cards[0].cardNo);
          ...
          pushHistoryWithPrePage(
            ownProps.history,
            `/expense/ic-card/list/${ownProps.reportId}/${ownProps.recordId}`
          );
        }
      });
  },
