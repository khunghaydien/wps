...
const RouteListItem = (props: Props) => {
  ...
  const onClickChangeRouteBtn = () => {
    const { recordDate, routeInfo, items } = values;
  ...
  const saveBtn = (
    <TextButton type="submit" onClick={props.handleSubmit}>
      {msg().Com_Btn_Save}
    </TextButton>
  );

  const editBtn = (
    <TextButton disabled={props.isSubmitted} onClick={props.onClickEditButton}>
      {msg().Com_Btn_Edit}
    </TextButton>
  );
  ...
  let navigationActions = [saveBtn];
  if (isNewRecord) {
    navigationActions = [saveBtn];
  } else if (reportDiscarded || reportClaimed || isUnderApprovedPreRequest) {
    navigationActions = [];
  } else if (readOnly) {
    navigationActions = [editBtn, actionBtn];
  }
  ...
  return (
    <Wrapper className={ROOT}>
      ...
      <Form className="main-content">
        ...
        {getExtendedItemArray(values.items[0])
          .filter((i) => i.id)
          .map(({ id, info, index }) => {
            if (!info) {
              return null;
