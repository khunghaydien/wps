...
export const actions = {
  ...
  delete:
    (recordId: string, isRequest?: boolean) =>
    (dispatch: AppDispatch): Promise<{ payload: any; type: string }> => {
      const recordIds = [recordId];
      return (
        isRequest ? deletePreRequestRecord(recordIds) : deleteRecord(recordIds)
    ...
    (): Promise<RecordCloneResponseType> => {
      return (
        isRequest
          ...
          : cloneRecord(recordIds, targetDates, numberOfDays, empId)
      ).then((result) => result);
    },
};
