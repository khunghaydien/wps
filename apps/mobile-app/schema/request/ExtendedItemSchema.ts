...
import { isHotelFee } from '../../../domain/models/exp/Record';
...
function validateEIIsRequired(isRequired, schema) {
  if (isRequired === 'true') {
    ...
    return schema.required(msg().Common_Err_Required);
  }
  return schema;
}

const getExtendedItemsValidators = (values?) => {
  if (values && isHotelFee(values.recordType)) {
    return {};
  }
